#!/usr/bin/env python3
"""
STEP 5 — Fairness Stress Test under Strict Ethnicity × Disease Balance (ETHNICITY)
...
Changes vs scFoundation version:
  ...
  [GF 3] DATASETS     : updated filenames + cell counts from step3b
  [GF 4] conda env    : scfoundation_gpu -> geneformer310
"""
...
OUTDIR      = BASE / "step5_outputs_ethnicity_geneformer"
...
def strict_ethnicity_balance_indices(ad, min_required=MIN_REQUIRED, seed=RANDOM_STATE):
    ...
    valid_eths = []
    for eth in tab[GROUP_KEY].unique():
        sub = tab[tab[GROUP_KEY] == eth]
        if sub["disease_binary"].nunique() == 2 and sub["count"].min() > 0:
            valid_eths.append(eth)
    if len(valid_eths) < 2:
...
def plot_learning_curves(lc_df, outdir):
    ...
    fig.suptitle("Learning Curves — CRC Ethnicity Fairness (Pilot, Geneformer)", fontsize=10, y=1.02)
    plt.tight_layout()
    out = outdir / "step5_learning_curves_ethnicity_geneformer.png"
...
t_start = time.time()
report_lines = ["STEP 5 -- FAIRNESS STRESS TEST (CRC ETHNICITY, Geneformer)\n" + "=" * 70 + "\n"]
all_lc_rows  = []
summary_rows = []
...
for ds_name, path in DATASETS.items():
    ...
    report_lines.append(f"  Worst ethnicity:   {cls_full['worst_ethnicity']} ({cls_full['worst_eth_accuracy']})")
    report_lines.append("  Per-ethnicity accuracy:")
    for k, v in sorted(cls_full["per_eth_accuracy"].items()):
        report_lines.append(f"    {k}: {v}")

    if cls_strict:
        report_lines.append(f"\nStrict balanced (min per group={min_g}, ethnicities={valid_eths}):")
        ...
        report_lines.append(f"  Worst ethnicity:   {cls_strict['worst_ethnicity']} ({cls_strict['worst_eth_accuracy']})")
        report_lines.append("  Per-ethnicity accuracy:")
        for k, v in sorted(cls_strict["per_eth_accuracy"].items()):
            report_lines.append(f"    {k}: {v}")
...
out_txt = OUTDIR / "step5_fairness_stress_test_ethnicity_geneformer.txt"
out_txt.write_text("\n".join(report_lines))
log(f"\nReport  -> {out_txt}")

out_csv = OUTDIR / "step5_summary_ethnicity_geneformer.csv"
...
if all_lc_rows:
    ...
    if lc_nonempty:
        ...
        out_lc_csv = OUTDIR / "step5_learning_curves_ethnicity_geneformer.csv"
