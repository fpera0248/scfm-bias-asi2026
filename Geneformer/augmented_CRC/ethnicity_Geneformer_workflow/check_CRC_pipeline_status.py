...
def expected_outputs(wf_dir, model, axis):
    ...
    def find_csv(step_prefix, csv_name):
        """Search all subdirs of wf_dir for csv_name under any dir matching step_prefix."""
        for d in wf_dir.iterdir():
            if not d.is_dir(): continue
            if d.name.startswith(step_prefix):
                p = d / csv_name
                if p.exists(): return p
                # also try any csv in that dir
                candidates = list(d.glob("*.csv"))
                if candidates: return candidates[0]  # at least something ran
        # fallback to guessed path
        return wf_dir / f"{step_prefix}_{suffix}" / csv_name

    axis_tag = "age" if axis == "AGE" else "ethnicity"
    checks["step4_csv"]       = find_csv("step4_external_validation", step4_csv)
    checks["step4a_csv"]      = find_csv("step4a_downstream",         step4a_csv)
    checks["step4b_csv"]      = find_csv("step4b_model_robustness",   step4b_csv)
    checks["step5_csv"]       = find_csv("step5_outputs",             step5_csv)
    checks["step6_csv"]       = find_csv("step6_outputs",             step6_csv)
    checks["step7_csv"]       = find_csv("step7_representation",      step7_csv)
    checks["step8_csv"]       = find_csv("step8_",                    step8_csv)
    checks["step8_worst_csv"] = find_csv("step8_",                    step8_worst)

    # step9: any dir starting with step9_ containing PNGs
    step9_path = next((d for d in wf_dir.iterdir()
                       if d.is_dir() and d.name.startswith("step9_")), wf_dir / step9_dir)
    checks["step9_dir_exists"] = step9_path
    checks["step9_has_pngs"]   = step9_path
