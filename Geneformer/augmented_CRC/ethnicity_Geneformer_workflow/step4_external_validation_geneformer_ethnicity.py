#!/usr/bin/env python3
"""
STEP 4 -- External Validation Disease + Cell-Type Classification (ETHNICITY)
Geneformer V2-316M embeddings

Changes vs step4 scFoundation ethnicity:
  [GF 1]  BASE path -> ethnicity_Geneformer_workflow
  [GF 2]  EMB_KEY   -> X_geneformer
  [GF 3]  PILOTS    -> _labeled_geneformer.h5ad filenames, 2143Each / 48Each / 2497
  [GF 4]  VALIDATION_FILE -> CRC_Eth_External_Validation_8572_geneformer.h5ad
  [GF 5]  OUTDIR    -> step4_external_validation_geneformer

All AR + EOS + evaluation logic preserved verbatim.
"""
...
VALIDATION_FILE = BASE / "CRC_Eth_External_Validation_8572_geneformer.h5ad"
# Also check step2a_embeddings subdir
if not VALIDATION_FILE.exists():
    VALIDATION_FILE = BASE / "step2a_embeddings" / "CRC_Eth_External_Validation_8572_geneformer.h5ad"   # [GF 4]

CSV_FILE = OUTDIR / "step4_external_validation_results.csv"
TXT_FILE = OUTDIR / "step4_external_validation_log.txt"
...
def load_pilot(name, path):
    ...
    log(f"  {name}: {ad.n_obs:,} cells, "
        f"{eth.nunique()} ethnicities, {ct.nunique()} cell types")
...
def load_validation(path):
    ...
    log(f"Validation: {ad.n_obs:,} cells, "
        f"{eth.nunique()} ethnicities, {ct.nunique()} cell types")
...
def apply_eos(X_train, y_train, g_train, target_group):
    ...
    if X_min.shape[0] < 10:
        log(f"    EOS: minority '{target_group}' has only "
            f"{X_min.shape[0]} cells -- skipping.")
...
def main():
    ...
    log("=" * 70)
    log("STEP 4 -- External Validation Classification (CRC ETHNICITY, Geneformer)")
    log("=" * 70)
    ...
    for pname, ppath in PILOTS.items():
        ...
        log(f"  Disease training subset: {X_pilot_dis.shape[0]:,} cells "
            f"(filtered to {sorted(DISEASE_GROUPS)})")
    ...
    log(f"\n{'='*70}")
    log(f"STEP 4 COMPLETE (CRC ETHNICITY, Geneformer)")
    log(f"  Results -> {CSV_FILE.name}")
    log(f"  Log     -> {TXT_FILE.name}")

    summary_cols = [
        "pilot", "model", "strategy",
