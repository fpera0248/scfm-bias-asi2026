#!/usr/bin/env python3
"""
STEP 4 — Downstream Disease Prediction (ETHNICITY) with AR + EOS
...
BASE   = Path("/oscar/home/fperalta/data/fperalta/Geneformer/augmented_CRC/ethnicity_Geneformer_workflow")
OUTDIR = BASE / "step4a_downstream_ethnicity"
OUTDIR.mkdir(exist_ok=True)

OUTFILE_TXT = OUTDIR / "step4a_downstream_results_ethnicity_AR_EOS_geneformer.txt"
OUTFILE_CSV = OUTDIR / "step4a_downstream_results_ethnicity_AR_EOS_geneformer.csv"
...
def evaluate_per_ethnicity_conditioned(model_template, X, y, g):
    results = {}
    for eth in np.unique(g):
        mask = g == eth
        if mask.sum() < 100 or len(np.unique(y[mask])) < 2:
            continue
        stratify = safe_stratify(y[mask])
        X_tr, X_te, y_tr, y_te = train_test_split(
            X[mask], y[mask], test_size=TEST_SIZE, stratify=stratify, random_state=RANDOM_STATE)
        ...
        try:
            ...
            results[eth] = (accuracy_score(y_te, preds), f1_score(y_te, preds, average="macro"))
        except ValueError:
            continue
    return results
...
def main():
    ...
    with open(OUTFILE_TXT, "w") as fh:
        fh.write("STEP 4 -- Downstream Disease Prediction (ETHNICITY) with AR + EOS\nGeneformer V2-316M\n" + "="*80 + "\n")
        for dname, fpath in DATASETS.items():
            ...
            for model_name, model_tmpl in MODEL_TEMPLATES.items():
                for strat_name, flags in STRATEGIES.items():
                    ...
                    eth_cond = {}
                    if strat_name == "Baseline" and model_name != "LinearSVM":
                        eth_cond = evaluate_per_ethnicity_conditioned(model_tmpl, X_d, y_d, g_d)
                    ...
                    for eth, (a, sf) in eth_cond.items():
                        row[f"eth_conditioned_{eth}_acc"] = a
                        row[f"eth_conditioned_{eth}_macro_f1"] = sf
                    rows.append(row)
