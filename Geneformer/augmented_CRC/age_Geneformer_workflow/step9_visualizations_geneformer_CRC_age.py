#!/usr/bin/env python3
"""
STEP 9 — Results Visualization (AGE, Geneformer V2-316M)
...
OUTDIR     = BASE / "step9_visualizations_age_geneformer"         # [AGE 1]
STEP3A_DIR = BASE / "benchmark_outputs_geneformer_age"
STEP4A_DIR = BASE / "step4a_downstream_geneformer_age"
STEP4B_DIR = BASE / "step4b_model_robustness_tests_age_geneformer"
STEP5_DIR  = BASE / "step5_outputs_age_geneformer"
STEP6_DIR  = BASE / "step6_outputs_age_geneformer"
STEP7_DIR  = BASE / "step7_representation_diagnostics_age_geneformer"
STEP8_DIR  = BASE / "step8_age_conditioned_disease_geneformer"
...
UMAP_DISPLAY = {
    "Proportional_2498":          "Proportional (2,495 cells — real only)",
    "BalancedAugmented_650Each": "Balanced Augmented (1,262/bin — scDesign3)",
    "BalancedUpsampled_650Each": "Balanced Upsampled (1,262/bin — real only)",
    "Downsampled_124Each":         "Downsampled (25/bin — real only)",
}
...
def fig1_scib():
    ...
    for metric, (_, short, title, subtitle) in SCIB_METRIC_MAP.items():
        ...
        if sub.empty:
            continue
        _single_bar_figure(
            title=f"Fig 1 — scIB Embedding Quality (Geneformer AGE)\n{title}",  # [AGE 5]
...
def fig2_step4a():
    path = STEP4A_DIR / "step4a_downstream_results_age_AR_EOS_geneformer.csv"  # [AGE 4]
    if not path.exists():
    ...
    for metric, panel_title, ylabel, fname in panels:
        ...
        fig.suptitle(f"Fig 2 — Disease Prediction Strategies (Geneformer AGE)\n{panel_title}",
...
def fig3_step4b():
    path = STEP4B_DIR / "step4b_results_age_labeled_geneformer.csv"     # [AGE 4]
    if not path.exists():
    ...
    for ds in datasets:
        ...
        fig, ax = plt.subplots(figsize=(14, 7))
        fig.suptitle(f"Fig 3 — Model Robustness (Geneformer AGE)\n"   # [AGE 5]
...
def fig4_learning_curves():
    path = STEP5_DIR / "step5_learning_curves_age_geneformer.csv"       # [AGE 4]
    if not path.exists():
    ...
    for ds in datasets:
        sub = df[df["dataset"] == ds]
        fig, ax = plt.subplots(figsize=(11, 7))
        fig.suptitle(f"Fig 4 — Learning Curves (Geneformer AGE)\n"     # [AGE 5]
                     f"Dataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig5_step6():
    path = STEP6_DIR / "step6_per_age_diagnostics_geneformer.csv"       # [AGE 4]
    if not path.exists():
    ...
    for metric in metrics:
        ...
        for grp in groups:
            ...
            fig.suptitle(f"Fig 5 — Per-Age Fairness Deltas (Geneformer AGE)\n"
...
def fig6_step7():
    path = STEP7_DIR / "step7_per_age_diagnostics_geneformer.csv"       # [AGE 4]
    if not path.exists():
    ...
    for metric in metrics:
        ...
        fig.suptitle(f"Fig 6 — Representation Quality (Geneformer AGE)\n{title}",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig7_step8():
    per_age_path = STEP8_DIR / "step8_per_age_disease_prediction_geneformer.csv"  # [AGE 4]
    worst_path   = STEP8_DIR / "step8_worst_age_bin_summary_geneformer.csv"
    ...
    fig.suptitle("Fig 7A — Age-Conditioned Disease Prediction (Geneformer AGE)\n"  # [AGE 5]
    ...
    if not worst.empty:
        ...
        fig.suptitle("Fig 7B — Worst-Performing Age Bin Accuracy (Geneformer AGE)",
                     fontsize=17, fontweight="bold", y=1.02)
    ...
    if "delta_acc_vs_prop" in underrep_rows.columns and not underrep_rows.empty:
        ...
        fig.suptitle(f"Fig 7C — Age Bin {UNDERREP_GROUP} Accuracy Delta vs Proportional (Geneformer AGE)",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig8_summary_heatmap():
    ...
    p = STEP4A_DIR / "step4a_downstream_results_age_AR_EOS_geneformer.csv"  # [AGE 4]
    if p.exists():
        df4  = pd.read_csv(p)
    ...
    p = STEP8_DIR / "step8_worst_age_bin_summary_geneformer.csv"
    if p.exists():
        for _, row in pd.read_csv(p).iterrows():
            rows.append({"dataset": row["dataset"], "Worst-Age (S8)": row.get("worst_acc", np.nan)})

    p = STEP8_DIR / "step8_per_age_disease_prediction_geneformer.csv"
    if p.exists():
        df8b = pd.read_csv(p)
    ...
    fig.suptitle("Fig 8 — Summary Heatmap (Geneformer AGE)\n"           # [AGE 5]
...
def main():
    print("\nSTEP 9 — Visualization Pipeline (AGE, Geneformer V2-316M)")  # [AGE 5]
