#!/usr/bin/env python3
"""
STEP 4 — External Validation Disease + Cell-Type Classification (AGE, Geneformer V2-316M)

Changes vs sex version:
  [AGE 1] BASE path       -> age_Geneformer_workflow
  [AGE 2] OUTPUT_BASE     -> CRC_Age_Pilot
  [AGE 3] PILOTS          -> age filenames (1262Each, 25Each, 2495)
  [AGE 4] VALIDATION_FILE -> CRC_Age_External_Validation_9402_geneformer.h5ad
  [AGE 5] OUTDIR          -> step4_external_validation_geneformer_age
  [AGE 6] SEX_COL         -> AGE_COL = "age_bin_10yr"
  [AGE 7] DISEASE_GROUPS  -> all age bins included (no subsetting)
"""
...
PILOTS = {
    ...
    "Downsampled_124Each":         LABELED_DIR / f"{OUTPUT_BASE}_Downsampled_124Each_labeled_geneformer.h5ad",
    "BalancedUpsampled_650Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedUpsampled_650Each_labeled_geneformer.h5ad",
    "BalancedAugmented_650Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedAugmented_650Each_labeled_geneformer.h5ad",
}

VALIDATION_FILE = BASE / "CRC_Age_External_Validation_9402_geneformer.h5ad"
if not VALIDATION_FILE.exists():
    VALIDATION_FILE = BASE / "step2a_embeddings" / "CRC_Age_External_Validation_9402_geneformer.h5ad"

CSV_FILE = OUTDIR / "step4_external_validation_results_age.csv"
TXT_FILE = OUTDIR / "step4_external_validation_log_age.txt"
...
def load_pilot(name, path):
    ...
    age         = canonicalize_age(ad.obs[age_col])
    disease_bin = ad.obs[DISEASE_COL].apply(make_binary_disease)
    ct          = ad.obs[CELLTYPE_COL].astype(str)
    log(f"  {name}: {ad.n_obs:,} cells, {age.nunique()} age bins, {ct.nunique()} cell types")
    return X, age.to_numpy(), disease_bin.to_numpy(), ct.to_numpy()
...
def load_validation(path):
    ...
    disease_bin = ad.obs[DISEASE_COL].apply(make_binary_disease)
    ct          = ad.obs[CELLTYPE_COL].astype(str)
    log(f"Validation: {ad.n_obs:,} cells, {age.nunique()} age bins, {ct.nunique()} cell types")
    log(f"  Age dist: {pd.Series(age).value_counts().sort_index().to_dict()}")
    return X, age.to_numpy(), disease_bin.to_numpy(), ct.to_numpy()
...
def main():
    ...
    log("=" * 70)
    log("STEP 4 -- External Validation Classification (CRC AGE, Geneformer)")
    log("=" * 70)

    log("\nLoading validation set ...")
    X_val, age_val, dis_val, ct_val = load_validation(VALIDATION_FILE)

    # All age bins used for disease task — no group subsetting needed   [AGE 7]
    valid_dis   = ~pd.isna(pd.Series(dis_val))
    X_val_dis   = X_val[valid_dis.values]
    age_val_dis = age_val[valid_dis.values]
    dis_val_dis = dis_val[valid_dis.values]
    ...
    for pname, ppath in PILOTS.items():
        ...
        log(f"Pilot: {pname}")
        try:
            X_pilot, age_pilot, dis_pilot, ct_pilot = load_pilot(pname, ppath)
        except (FileNotFoundError, RuntimeError) as err:
            log(f"  SKIP: {err}")
            continue

        valid_pilot   = ~pd.isna(pd.Series(dis_pilot))
        X_pilot_dis   = X_pilot[valid_pilot.values]
        age_pilot_dis = age_pilot[valid_pilot.values]
        dis_pilot_dis = dis_pilot[valid_pilot.values]
        ...
        for model_name, model_tmpl in MODEL_TEMPLATES.items():
            for strat_name, flags in STRATEGIES.items():
                ...
                dres = evaluate_external(
                    model_tmpl,
                    X_pilot_dis, dis_pilot_dis, age_pilot_dis,
                    X_val_dis,   dis_val_dis,   age_val_dis,
                    use_ar=flags["use_ar"], use_eos=flags["use_eos"],
                )

                cres = evaluate_external(
                    model_tmpl,
                    X_pilot, ct_pilot, age_pilot,
                    X_val,   ct_val,   age_val,
                    use_ar=flags["use_ar"], use_eos=flags["use_eos"],
                )
                ...
                if dres is not None:
                    row.update({
                        ...
                        "disease_worst_age_bin": dres["worst_group"],
                ...
                if cres is not None:
                    row.update({
                        ...
                        "celltype_worst_age_bin": cres["worst_group"],
    ...
    log(f"\n{'='*70}")
    log(f"STEP 4 COMPLETE (CRC AGE, Geneformer)")
