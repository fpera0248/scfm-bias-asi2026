#!/usr/bin/env python3
"""
STEP 4a — Downstream Disease Prediction (AGE) with AR + EOS
...
Changes vs sex version:
  ...
  [AGE 4] GROUP_COL     -> age_bin_10yr instead of sex
...
OUTPUT_BASE = "CRC_Age_Pilot"                                    # [AGE 2]
...
def extract_features(adata, label_col, grp_col):
    ...
    g = canonicalize_age(adata.obs[grp_col])
...
def main():
    ...
    with open(OUTFILE_TXT, "w") as fh:
        fh.write("STEP 4a -- Downstream Disease Prediction (AGE) with AR + EOS\nGeneformer V2-316M\n" + "="*80 + "\n")
        for dname, fpath in DATASETS.items():
            ...
            grp_col = detect_age_col(adata.obs, fpath)           # [AGE 4]
            ...
            for model_name, model_tmpl in MODEL_TEMPLATES.items():
                for strat_name, flags in STRATEGIES.items():
                    ...
                    row = {
                        ...
                        "disease_worst_age_bin": worst_grp, "disease_worst_age_bin_acc": worst_acc,  # [AGE 4]
                        "celltype_accuracy": ct_acc, "celltype_macro_f1": ct_f1,
                        "celltype_worst_age_bin": ct_worst_grp, "celltype_worst_age_bin_acc": ct_worst_acc,
                        "random_baseline_acc": rand_acc, "random_baseline_f1": rand_f1, "n_cells": len(y_d),
                    }
                    for grp, a in per_group.items():
                        row[f"per_age_disease_acc_{grp}"] = a                         # [AGE 4]
                    rows.append(row)
    ...
    print("\n" + df[["dataset","model","strategy","disease_accuracy","disease_macro_f1","disease_worst_age_bin_acc","celltype_accuracy"]].to_string(index=False))  # [AGE 4]
