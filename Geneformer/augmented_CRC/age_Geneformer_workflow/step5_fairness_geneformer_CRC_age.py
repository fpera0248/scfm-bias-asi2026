...
OUTDIR      = BASE / "step5_outputs_age_geneformer"              # [AGE 4]
...
OUTPUT_BASE = "CRC_Age_Pilot"                                    # [AGE 1]
...
def plot_learning_curves(lc_df, outdir):
    ...
    fig.suptitle("Learning Curves — CRC Age Fairness (Pilot, Geneformer)", fontsize=10, y=1.02)  # [AGE 5]
    plt.tight_layout()
    out = outdir / "step5_learning_curves_age_geneformer.png"
...
t_start = time.time()
report_lines = ["STEP 5 -- FAIRNESS STRESS TEST (CRC AGE, Geneformer)\n" + "=" * 70 + "\n"]  # [AGE 5]
all_lc_rows  = []
summary_rows = []
...
out_txt = OUTDIR / "step5_fairness_stress_test_age_geneformer.txt"
out_txt.write_text("\n".join(report_lines))
log(f"\nReport  -> {out_txt}")

out_csv = OUTDIR / "step5_summary_age_geneformer.csv"
...
if all_lc_rows:
    ...
    if lc_nonempty:
        ...
        out_lc_csv = OUTDIR / "step5_learning_curves_age_geneformer.csv"
