#!/usr/bin/env python3
"""
STEP 9 — Results Visualization (ETHNICITY, Geneformer V2-316M)
...
PALETTE = {
    "Proportional_2500":          "#4C72B0",
    "BalancedAugmented_779Each": "#DD8452",
    "BalancedUpsampled_779Each": "#55A868",
    "Downsampled_92Each":         "#C44E52",
}

SHORT_LABELS = {
    "Proportional_2500":          "Proportional\n(2,497 cells)",
    "BalancedAugmented_779Each": "scDesign3\nAugmented\n(2,143/group)",
    "BalancedUpsampled_779Each": "Upsampled\n(2,143/group)",
    "Downsampled_92Each":         "Downsampled\n(48/group)",
}
...
UMAP_DATASETS = {
    "Proportional_2500":          "AIDA_Ethnicity_Pilot_Proportional_2500_ETHNICITY",
    "BalancedAugmented_779Each": "AIDA_Ethnicity_Pilot_BalancedAugmented_779Each_ETHNICITY",
    "BalancedUpsampled_779Each": "AIDA_Ethnicity_Pilot_BalancedUpsampled_779Each_ETHNICITY",
    "Downsampled_92Each":         "AIDA_Ethnicity_Pilot_Downsampled_92Each_ETHNICITY",
}

UMAP_DISPLAY = {
    "Proportional_2500":          "Proportional (2,497 cells — real only)",
    "BalancedAugmented_779Each": "Balanced Augmented (2,143/group — scDesign3)",
    "BalancedUpsampled_779Each": "Balanced Upsampled (2,143/group — real only)",
    "Downsampled_92Each":         "Downsampled (48/group — real only)",
}
...
def fig2_step4a():
    ...
    for metric, panel_title, ylabel, fname in panels:
        ...
        ax.set_title(f"Dataset: Proportional (2,497 cells)   {ylabel}",
...
def fig7_step8():
    ...
    if "delta_acc_vs_prop" in na_rows.columns and not na_rows.empty:
        na_plot = na_rows[na_rows["dataset"] != "Proportional_2500"].copy()
        na_plot = na_plot.set_index("dataset").reindex(
            [d for d in DATASET_ORDER if d != "Proportional_2500"]).reset_index()
        fig, ax = plt.subplots(figsize=(max(9, len(na_plot) * 2.6), 7))
        fig.suptitle("Fig 7C — Native American Accuracy Delta vs Proportional (Geneformer)",
                     fontsize=17, fontweight="bold", y=1.02)
        ...
        ax.set_ylabel("Delta Accuracy vs Proportional_2500 Baseline", fontsize=14, labelpad=10)
        ax.yaxis.grid(True, alpha=0.3, zorder=0)
        plt.tight_layout(pad=2.5)
...
def main():
    print("\nSTEP 9 — Visualization Pipeline (ETHNICITY, Geneformer V2-316M)")
