...
Changes vs ethnicity version:
  ...
  [SEX 2] OUTPUT_BASE     -> AIDA_Age_Pilot
  [SEX 3] PILOTS          -> sex filenames (1413Each, 586Each, 1999)
  [SEX 4] VALIDATION_FILE -> AIDA_Age_External_Validation_10000_geneformer.h5ad
  [SEX 5] OUTDIR          -> step4_external_validation_geneformer_sex
  [SEX 6] ETHNICITY_COL   -> AGE_BIN_COL = "age_bin_10yr"
...
OUTDIR      = BASE / "step4_external_validation_geneformer_age"   # [SEX 5]
OUTDIR.mkdir(exist_ok=True)

OUTPUT_BASE = "AIDA_Age_Pilot"                                      # [SEX 2]
...
VALIDATION_FILE = BASE / "AIDA_Age_External_Validation_10000_geneformer.h5ad"  # [SEX 4]
...
AGE_BIN_COL      = "age_bin_10yr"                                              # [SEX 6]

DISEASE_GROUPS = {"10_19", "20_29", "30_39", "40_49", "50_59", "60_69", "70_79"}                               # [SEX 7]
...
def canonicalize_age_bin(series):                                     # [SEX 6]
...
def load_validation(path):
    ...
    disease_bin = ad.obs[DISEASE_COL].apply(make_binary_disease)
    ct          = ad.obs[CELLTYPE_COL].astype(str)
    log(f"Validation: {ad.n_obs:,} cells, {age_bin.nunique()} age groups, {ct.nunique()} cell types")
    log(f"  Age dist: {pd.Series(age_bin).value_counts().to_dict()}")
    return X, age_bin.to_numpy(), disease_bin.to_numpy(), ct.to_numpy()
...
def main():
    ...
    log("=" * 70)
    log("STEP 4 -- External Validation Classification (AGE, Geneformer)")  # [SEX]
    log("=" * 70)
    ...
    for pname, ppath in PILOTS.items():
        ...
        age_pilot_dis = age_pilot[pilot_disease_mask]
            ...
            for strat_name, flags in STRATEGIES.items():
                ...
                if dres is not None:
                    row.update({
                        ...
                        "disease_worst_age": dres["worst_group"],
                ...
                if cres is not None:
                    row.update({
                        ...
                        "celltype_worst_age": cres["worst_group"],
    ...
    log(f"\n{'='*70}")
    log(f"STEP 4 COMPLETE (AGE, Geneformer)")
