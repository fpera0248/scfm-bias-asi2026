#!/usr/bin/env python3
"""
STEP 8 — Sex-Conditioned Disease Prediction (AGE, Geneformer V2-316M)
...
Changes vs ethnicity version:
  ...
  [SEX 5] Reference baseline -> Proportional_2495
...
def main():
    log("=" * 70)
    log("STEP 8 -- Sex-Conditioned Disease Prediction (AGE, Geneformer)")
    log("=" * 70)
    ...
    out_per_age = OUTDIR / "step8_per_age_disease_prediction_geneformer.csv"
    per_age_df.to_csv(out_per_age, index=False)
    log(f"\nPer-sex CSV -> {out_per_age}")

    out_worst = OUTDIR / "step8_worst_age_summary_geneformer.csv"
    worst_age_df.to_csv(out_worst, index=False)
    log(f"Worst-sex CSV -> {out_worst}")

    out_underrep = OUTDIR / "step8_underrepresented_age_summary_geneformer.csv"
    ...
    lines = [
        "STEP 8 -- AGE-CONDITIONED DISEASE PREDICTION (AGE, Geneformer)",
        "=" * 70, "",
        ...
        "",
        "=" * 70,
        "FULL PER-AGE RESULTS",
        "=" * 70,
        per_age_df[["dataset", "age_bin_10yr", "accuracy", "balanced_accuracy",
                    "macro_f1", "delta_acc_vs_prop", "n_test_cells"]].to_string(index=False),
        "",
        "=" * 70,
        "WORST-AGE PER DATASET",
        "=" * 70,
