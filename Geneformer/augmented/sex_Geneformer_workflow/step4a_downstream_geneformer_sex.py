...
BASE   = Path("/data/Geneformer/augmented/sex_Geneformer_workflow")
OUTDIR = BASE / "step4a_downstream_sex"
OUTDIR.mkdir(exist_ok=True)

OUTFILE_TXT = OUTDIR / "step4a_downstream_results_sex_AR_EOS_geneformer.txt"
OUTFILE_CSV = OUTDIR / "step4a_downstream_results_sex_AR_EOS_geneformer.csv"
...
def detect_sex_col(obs, fname):                                  # [SEX 4]
...
def extract_features(adata, label_col, grp_col):
    ...
    g = canonicalize_sex(adata.obs[grp_col])
...
def main():
    ...
    with open(OUTFILE_TXT, "w") as fh:
        ...
        for dname, fpath in DATASETS.items():
            ...
            grp_col = detect_sex_col(adata.obs, fpath)           # [SEX 4]
            ...
            for model_name, model_tmpl in MODEL_TEMPLATES.items():
                for strat_name, flags in STRATEGIES.items():
                    ...
                    row = {
                        ...
                        "celltype_worst_sex": ct_worst_grp, "celltype_worst_sex_acc": ct_worst_acc,
