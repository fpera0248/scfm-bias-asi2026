#!/usr/bin/env Rscript
# ============================================================
# STEP 0B: SEX AUGMENTATION WITH scDesign3 (PILOT, v5)
# Geneformer workflow — augmented/sex_Geneformer_workflow
# ============================================================
# * Group by sex (female / male)
# * Majority (male): real-only, down-sampled to TARGET_PER_BIN
# * Minority (female): real down-sampled proportionally, then
...
#       "cell_type + sex + cell_type:sex
#        + self_reported_ethnicity + age_bin_10yr + offset(log(library))"
#       self_reported_ethnicity / age_bin_10yr soft-degrade if <2 levels
#   - sigma_formula = "cell_type"
#   - corr_formula  = "cell_type"
#   - copula        = "gaussian"
#
# V5 CHANGES vs. augmentedv4 sex script:
#   [V5 1]  VALIDATION_H5AD: ILD_Sex_External_Validation_5000.h5ad
...
#   [V5 5]  family_use = "zinb" at all 4 sites.
#   [V5 6]  mu_formula: + self_reported_ethnicity + age_bin_10yr + offset(log(library))
#   [V5 7]  construct_data other_covariates: c(BIN_COL, "self_reported_ethnicity",
#           "age_bin_10yr", "library")
#   [V5 8]  build_mu_formula() soft-degrade for self_reported_ethnicity and age_bin_10yr.
#   [V5 9]  add_age_bins() helper: parses development_stage -> age_bin_10yr.
#   [GF 1]  BASE PATH: augmentedv4/sex_scfoundation_workflow
#                   -> Geneformer/augmented/sex_Geneformer_workflow
#
...
VALIDATION_H5AD <- "ILD_Sex_External_Validation_5000.h5ad"   # [V5 1]
OUTPUT_BASE     <- "ILD_Sex_Pilot"
CKPTDIR         <- "checkpoints_sex_augmentation_pilot_v5"
LOGFILE         <- "augmentation_sex_pilot_v5_log.txt"

BIN_COL      <- "sex"
CELLTYPE_COL <- "cell_type"
AGE_COL      <- "development_stage"   # [V5 9]
AGE_BIN_COL  <- "age_bin_10yr"        # [V5 9]
...
# [V5 9] Age bin config matching age workflow
...
heartbeat("\n=== STARTING SEX AUGMENTATION v5 (Geneformer workflow) ===\n", TRUE)
...
assert_raw_counts <- function(sce_obj, assay_name = "counts") {
  ...
  if (inherits(mat, "dgCMatrix")) {
    ...
    vals_all <- mat[cbind(sample.int(nr, samp_n, replace=TRUE), sample.int(nc, samp_n, replace=TRUE))]
  }
...
# ============================================================
# 1. AUTO-DETECT SEX GROUP STRUCTURE
# ============================================================

heartbeat("\n=== AUTO-DETECTING SEX GROUP STRUCTURE ===\n", TRUE)
...
sex_raw    <- tolower(trimws(as.character(colData(sce_detect)[[BIN_COL]])))
valid      <- !(sex_raw %in% UNKNOWN_VALUES) & !is.na(sex_raw) & sex_raw %in% c("female", "male")
...
heartbeat("Sex group sizes (validation-excluded):\n", TRUE)
...
sex_full   <- tolower(trimws(as.character(colData(sce_full)[[BIN_COL]])))
valid_full <- !(sex_full %in% UNKNOWN_VALUES) & !is.na(sex_full) & sex_full %in% c("female", "male")
...
sex_sample   <- tolower(trimws(as.character(colData(sce_sample)[[BIN_COL]])))
valid_sample <- !(sex_sample %in% UNKNOWN_VALUES) & !is.na(sex_sample) & sex_sample %in% c("female", "male")
...
load_bin_fresh <- function(bin_label) {
  heartbeat(sprintf("Loading sex group '%s' (HVG subset)...", bin_label), TRUE)
  ...
  sex_raw_vec <- tolower(trimws(as.character(colData(sce_raw)[[BIN_COL]])))
  valid_sex   <- !(sex_raw_vec %in% UNKNOWN_VALUES) & !is.na(sex_raw_vec) &
                 sex_raw_vec %in% c("female", "male")
  sce_raw     <- sce_raw[, valid_sex]
  ...
  sex_vec  <- tolower(trimws(as.character(colData(sce_raw)[[BIN_COL]])))
  sce_bin  <- sce_raw[, sex_vec == bin_label]
  if (ncol(sce_bin) == 0) stop(sprintf("No cells for sex group '%s'", bin_label))

  common  <- intersect(hvg_names, rownames(sce_bin))
  if (length(common) == 0) stop(sprintf("No HVGs in sex group '%s'", bin_label))
...
# ============================================================
# [V5 8] BUILD_MU_FORMULA for SEX
# Primary: sex interaction. Controls: self_reported_ethnicity + age_bin_10yr.
# ============================================================

build_mu_formula <- function(train_sce) {
  formula_parts <- c("cell_type", "sex", "cell_type:sex")
  other_extra   <- character(0)

  for (covar in c(ETH_COL, AGE_BIN_COL)) {
...
# ============================================================
# 9. AUGMENTATION FOR ONE SEX GROUP
# ============================================================

augment_one_bin <- function(bin_label) {
  ckpt_file <- file.path(CKPTDIR, paste0("sex_", gsub("[^a-zA-Z0-9_]", "_", bin_label), ".rds"))
  ...
  while (remaining > 0) {
    ...
    while (attempt <= MAX_RETRIES && !success) {
      ...
      sex_vals    <- unique(as.character(colData(sub_train)[[BIN_COL]]))
      has_sex_var <- length(sex_vals) > 1L
      if (!has_sex_var) {
        stop(sprintf("[Tier 1 gate] No sex variance for '%s'.", bin_label))
      }

      ct_sex_tab <- table(CT  = as.character(colData(sub_train)[[CELLTYPE_COL]]),
                          Grp = as.character(colData(sub_train)[[BIN_COL]]))
      single_ct  <- names(rowSums(ct_sex_tab > 0))[rowSums(ct_sex_tab > 0) < 2L]
      if (length(single_ct) > 0) {
        stop(sprintf("[Tier 1 gate] %d CT(s) in only 1 sex group: %s",
      ...
      heartbeat(" [Tier 1] sex+interaction+ethnicity+age+offset(lib), corr=cell_type [V5]\n", TRUE)
...
# ============================================================
# 10. PROCESS BOTH SEX GROUPS
# ============================================================

heartbeat("\n=== PROCESSING BOTH SEX GROUPS ===\n", TRUE)
bin_objects <- list()
for (b in all_bins) {
...
out_full <- paste0(OUTPUT_BASE, "_Full_BalancedAugmented_SEX.h5ad")
...
out_real <- paste0(OUTPUT_BASE, "_RealOnly_SEX.h5ad")
...
out_syn <- paste0(OUTPUT_BASE, "_SyntheticOnly_SEX.h5ad")
zellkonverter::writeH5AD(rebuild_for_write(sce_syn, drop_all_na = TRUE), out_syn, compression = "gzip")
heartbeat(sprintf("Wrote: %s\n", out_syn), TRUE)

out_sum <- paste0(OUTPUT_BASE, "_Summary_SEX.csv")
summary_df <- as.data.frame(table(Sex = colData(sce_combined)[[BIN_COL]],
...
sex_counts_fn <- function(sce_obj) {
  sx <- tolower(trimws(as.character(colData(sce_obj)[[BIN_COL]])))
  table(sx)
}
...
sex_raw_prop <- tolower(trimws(as.character(colData(sce_for_prop)[[BIN_COL]])))
valid_prop   <- !(sex_raw_prop %in% UNKNOWN_VALUES) & !is.na(sex_raw_prop) &
                sex_raw_prop %in% c("female", "male")
...
out_prop <- paste0(OUTPUT_BASE, "_Proportional_", sum(prop_targets), "_SEX.h5ad")
zellkonverter::writeH5AD(rebuild_for_write(sce_prop), out_prop, compression = "gzip")
heartbeat(sprintf("Wrote Proportional: %s\n", out_prop), TRUE)
rm(sce_for_prop); gc()

out_bal_aug <- paste0(OUTPUT_BASE, "_BalancedAugmented_", TARGET_PER_BIN, "Each_SEX.h5ad")
...
out_up  <- paste0(OUTPUT_BASE, "_BalancedUpsampled_", TARGET_PER_BIN, "Each_SEX.h5ad")
...
out_down <- paste0(OUTPUT_BASE, "_Downsampled_", DOWN_TARGET, "Each_SEX.h5ad")
...
heartbeat("\n=== SEX AUGMENTATION V5 (Geneformer workflow) COMPLETE ===\n", TRUE)
...
heartbeat("\nAll done! Check augmentation_sex_pilot_v5_log.txt for details.\n", TRUE)