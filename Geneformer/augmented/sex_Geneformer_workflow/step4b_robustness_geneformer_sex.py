#!/usr/bin/env python3
"""
STEP 4b — Robustness Stress Test (SEX, Pilot)
...
OUTDIR = BASE / "step4b_model_robustness_tests_sex_geneformer"   # [SEX 6]
OUTDIR.mkdir(exist_ok=True)

CSV_FILE = OUTDIR / "step4b_results_sex_labeled_geneformer.csv"
TXT_FILE = OUTDIR / "step4b_full_log_sex_labeled_geneformer.txt"

logger = logging.getLogger("STEP4B_SEX_GF")
...
logger.info("========== STEP 4b START (SEX, Geneformer) ==========")
...
KNOWN_GROUPS = ["female", "male"]                                # [SEX 5]
...
for dname, path in DATASETS.items():
    ...
    ad.obs["_sex"] = canonicalize_sex(ad.obs[grp_col])
    ...
    logger.info(f"  Sex dist: {pd.Series(g0).value_counts().to_dict()}")
...
logger.info("========== STEP 4b COMPLETE (SEX, Geneformer) ==========")
...
summary = (
    df.groupby(["dataset", "model", "strategy"])[["accuracy", "macro_f1", "worst_sex_acc"]]
