#!/usr/bin/env python3
"""
STEP 0c — External Validation Set Construction (SEX, Geneformer workflow)

Changes vs. augmentedv4 version:
  [GF 1] BASE path updated to Geneformer/augmented/sex_Geneformer_workflow
  [GF 2] PILOT_FILES = [] — no prior pilot. Run before step0b.
  [GF 3] CELLS_PER_GROUP = 2500, 2 groups = 5000 total.
...
PILOT_FILES: list[Path] = []   # [GF 2] no prior pilot
OUT_FILE = BASE / "ILD_Sex_External_Validation_5000.h5ad"
LOGFILE  = BASE / "step0c_external_validation_sex_log.txt"
...
def collect_pilot_barcodes(pilot_files):
    ...
    for f in pilot_files:
        ...
        if SOURCE_COL in ad.obs.columns:
            real_barcodes = ad.obs.index[(ad.obs[SOURCE_COL] == "real").to_numpy()].tolist()
        else:
...
def main():
    ...
    log("=" * 70)
    log("STEP 0c -- External Validation Set Construction (SEX, Geneformer)")
    log("=" * 70)
    ...
    if GROUP_COL not in adata.obs.columns:
        raise RuntimeError(f"Missing '{GROUP_COL}' in obs.")
    if DISEASE_COL not in adata.obs.columns:
        raise RuntimeError(f"Missing '{DISEASE_COL}' in obs.")

    adata.obs[GROUP_COL] = adata.obs[GROUP_COL].astype(str).str.strip().str.lower()

    if pilot_barcodes:
        n_before  = adata.n_obs
        adata     = adata[~adata.obs.index.isin(pilot_barcodes)].copy()
        log(f"\nExcluded {n_before - adata.n_obs:,} pilot cells; {adata.n_obs:,} remaining")
    else:
        log(f"\nNo pilot barcodes to exclude; using full cohort ({adata.n_obs:,} cells)")

    valid_disease = ~adata.obs[DISEASE_COL].isna()
    if (~valid_disease).sum() > 0:
        log(f"  Dropping {(~valid_disease).sum():,} cells with missing disease label")
        adata = adata[valid_disease].copy()

    adata.obs["disease_binary"] = adata.obs[DISEASE_COL].apply(to_binary_disease)

    log(f"\nAvailable cells per sex group:")
    ...
    if len(insufficient) > 0:
        raise RuntimeError(f"Cannot sample {CELLS_PER_GROUP:,} from: {insufficient.to_dict()}")

    log(f"\nStratified sampling {CELLS_PER_GROUP:,} cells per sex group...")
    ...
    for grp in sorted(counts.index):
        ...
        target_disease = min(round(CELLS_PER_GROUP * n_disease_avail / n_total_avail), n_disease_avail)
        target_normal  = min(CELLS_PER_GROUP - target_disease, n_normal_avail)

        chosen = []
        if target_disease > 0:
            chosen.extend(rng.choice(grp_idx[disease_mask], size=target_disease, replace=False).tolist())
        if target_normal > 0:
            chosen.extend(rng.choice(grp_idx[normal_mask], size=target_normal, replace=False).tolist())
        ...
        if shortfall > 0:
            ...
            if len(remaining_idx) >= shortfall:
                chosen.extend(rng.choice(remaining_idx, size=shortfall, replace=False).tolist())
    ...
    log(f"\nDisease distribution by sex (binary):")
    ...
    if pilot_barcodes:
        ...
        if overlap:
            raise RuntimeError(f"Validation set overlaps with pilot: {len(overlap)} cells.")
    ...
    log(f"\n{'='*70}")
    log("STEP 0c COMPLETE (SEX, Geneformer)")
