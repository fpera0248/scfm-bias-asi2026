#!/usr/bin/env python3
"""
STEP 3b — kNN Disease Label Propagation (SEX, Geneformer V2-316M)
...
Changes vs ethnicity version:
  ...
  [SEX 4] ETHNICITY_COL -> SEX_COL = "sex"
...
DATASETS = {                                                     # [SEX 3]
    "BalancedAugmented_1413Each": BASE / f"{OUTPUT_BASE}_BalancedAugmented_1413Each_SEX_geneformer.h5ad",
    "Proportional_1999":          BASE / f"{OUTPUT_BASE}_Proportional_1999_SEX_geneformer.h5ad",
    "BalancedUpsampled_1413Each": BASE / f"{OUTPUT_BASE}_BalancedUpsampled_1413Each_SEX_geneformer.h5ad",
    "Downsampled_586Each":        BASE / f"{OUTPUT_BASE}_Downsampled_586Each_SEX_geneformer.h5ad",
}
...
def make_output_path(dname: str) -> Path:
    return OUTDIR / f"{OUTPUT_BASE}_{dname}_labeled_geneformer.h5ad"
...
def propagate_labels(adata, dname: str) -> sc.AnnData:
    ...
    if SEX_COL in adata.obs.columns:                            # [SEX 4]
        log(f"  Sex breakdown:")
...
def main():
    ...
    log("=" * 70)
    log("STEP 3b -- kNN Disease Label Propagation (SEX, Geneformer)")  # [SEX 5]
    log("=" * 70)
    log(f"KNN_K       = {KNN_K}")
    ...
    log(f"\n{'='*70}")
    log("STEP 3b COMPLETE (SEX, Geneformer)")
