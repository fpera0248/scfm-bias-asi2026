#!/usr/bin/env python3
"""
STEP 5 — Fairness Stress Test under Strict Sex × Disease Balance (SEX)
...
Changes vs ethnicity version:
  ...
  [SEX 4] Output files -> *_sex_geneformer suffix
  [SEX 5] Log strings updated for sex context
"""
...
OUTDIR      = BASE / "step5_outputs_sex_geneformer"              # [SEX 4]
...
OUTPUT_BASE = "ILD_Sex_Pilot"                                    # [SEX 1]
...
def load_dataset(path):
    ...
    if GROUP_KEY not in ad.obs.columns:
        raise RuntimeError(f"Missing '{GROUP_KEY}' column in {path.name}")
    ad.obs[GROUP_KEY] = canonicalize_sex(ad.obs[GROUP_KEY])
...
def evaluate_classifier(X, y, g, seed=RANDOM_STATE):
    ...
    return {
        ...
        "worst_sex":         str(worst_grp),
        "worst_sex_accuracy": round(float(worst_acc), 4) if pd.notna(worst_acc) else np.nan,
        "per_sex_accuracy":  {k: round(float(v), 4) for k, v in per_grp.items()},
        "n_train":           int(X_tr.shape[0]),
        "n_test":            int(X_te.shape[0]),
    }

def strict_sex_balance_indices(ad, min_required=MIN_REQUIRED, seed=RANDOM_STATE):  # [SEX 2]
...
def plot_learning_curves(lc_df, outdir):
    ...
    fig.suptitle("Learning Curves — Sex Fairness (Pilot, Geneformer)", fontsize=10, y=1.02)
    plt.tight_layout()
    out = outdir / "step5_learning_curves_sex_geneformer.png"
...
t_start = time.time()
report_lines = ["STEP 5 -- FAIRNESS STRESS TEST (SEX, Geneformer)\n" + "=" * 70 + "\n"]
all_lc_rows  = []
summary_rows = []
...
for ds_name, path in DATASETS.items():
    ...
    report_lines.append(f"  Worst sex:         {cls_full['worst_sex']} ({cls_full['worst_sex_accuracy']})")
    report_lines.append("  Per-sex accuracy:")
    for k, v in sorted(cls_full["per_sex_accuracy"].items()):
    ...
    if cls_strict:
        ...
        report_lines.append(f"  Worst sex:         {cls_strict['worst_sex']} ({cls_strict['worst_sex_accuracy']})")
        report_lines.append("  Per-sex accuracy:")
        for k, v in sorted(cls_strict["per_sex_accuracy"].items()):
            report_lines.append(f"    {k}: {v}")
...
out_txt = OUTDIR / "step5_fairness_stress_test_sex_geneformer.txt"
out_txt.write_text("\n".join(report_lines))
log(f"\nReport  -> {out_txt}")

out_csv = OUTDIR / "step5_summary_sex_geneformer.csv"
...
if all_lc_rows:
    ...
    if lc_nonempty:
        ...
        out_lc_csv = OUTDIR / "step5_learning_curves_sex_geneformer.csv"
