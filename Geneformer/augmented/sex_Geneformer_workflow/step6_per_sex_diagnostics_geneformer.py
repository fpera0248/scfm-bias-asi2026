#!/usr/bin/env python3
"""
STEP 6 — Per-Sex Fairness Diagnostics (SEX, Geneformer V2-316M)
...
OUTDIR      = BASE / "step6_outputs_sex_geneformer"             # [SEX 5]
...
def accuracy_per_sex(ad, seed=RANDOM_STATE):
    ...
    for grp in np.unique(g_te):
        ...
        if m.sum() < 5:
            continue
        per_sex[grp] = round(float(accuracy_score(y_te[m], preds[m])), 4)
    return per_sex
...
def main():
    log("=" * 70)
    log("STEP 6 -- Per-Sex Fairness Diagnostics (SEX, Geneformer)")
    log("=" * 70)
    ...
    for label, ad in datasets.items():
        ...
        for other in [mix_df, acc_df, ari_df]:
            if not other.empty:
                df = (df.merge(other, on="sex", how="outer")
    ...
    out_csv = OUTDIR / "step6_per_sex_diagnostics_geneformer.csv"
    pivot.to_csv(out_csv)
    log(f"\nCSV saved -> {out_csv}")

    out_txt = OUTDIR / "step6_per_sex_diagnostics_report_geneformer.txt"
    lines = [
        "STEP 6 -- PER-SEX FAIRNESS DIAGNOSTICS (SEX, Geneformer)",
        "=" * 70, "",
        "SILHOUETTE: higher = better disease separation per sex group.",
        "KNN MIXING: 0.3-0.7 target; random expected ~0.50 with 2 groups.",
