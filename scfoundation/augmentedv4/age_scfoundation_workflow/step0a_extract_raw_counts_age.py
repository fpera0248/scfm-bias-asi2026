...
This script:
  ...
  • Validates & prepares all metadata columns needed by scDesign3.
  • Extracts raw (unnormalized) counts from `adata.raw`.
  • Writes a clean raw-counts file ready for the AGE-based
    augmentation step (Step 0B).

Output:
  InterstitialLungDisease_RawCounts_AGE.h5ad
"""
...
OUTPUT_FILE = "InterstitialLungDisease_RawCounts_AGE.h5ad"   # [AGE 1]
...
try:
    ...
    # Extract first contiguous digit sequence (e.g. "45-year-old..." -> "45")
    age_digits = age_text.str.extract(r"(\d+)", expand=False)
    age_num    = pd.to_numeric(age_digits, errors="coerce")

    # Keep only cells with parseable age within bin range [10, 90)
    ...
    heartbeat("\n Ready for Step 0B (scDesign3 AGE-based augmentation)\n", True)
