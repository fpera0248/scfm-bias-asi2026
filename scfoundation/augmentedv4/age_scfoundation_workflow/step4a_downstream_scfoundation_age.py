#!/usr/bin/env python3
"""STEP 4a — Downstream Disease Prediction (AGE, scFoundation)"""
...
BASE   = Path("/oscar/home/fperalta/data/fperalta/scfoundation/augmentedv4/age_scfoundation_workflow")
...
OUTFILE_CSV = OUTDIR / "step4a_downstream_results_age_AR_EOS_scfoundation.csv"

OUTPUT_BASE = "ILD_Age_Pilot"
LABELED_DIR = BASE / "step3b_labeled"
DATASETS = {
    "BalancedAugmented_1262Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedAugmented_1262Each_labeled_scfoundation.h5ad",
    "Proportional_2495":          LABELED_DIR / f"{OUTPUT_BASE}_Proportional_2495_labeled_scfoundation.h5ad",
    "BalancedUpsampled_1262Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedUpsampled_1262Each_labeled_scfoundation.h5ad",
    "Downsampled_25Each":        LABELED_DIR / f"{OUTPUT_BASE}_Downsampled_25Each_labeled_scfoundation.h5ad",
}

EMB_KEY = "X_scfoundation"; DISEASE_COL = "disease"; CELLTYPE_COL = "cell_type"
AGE_BIN_COL_CANDIDATES = ["age_bin_10yr"]
...
def detect_age_col(obs, fname):
    ...
    raise RuntimeError(f"No age column in {fname}.")
...
def extract_features(adata, label_col, grp_col):
    ...
    y = adata.obs[label_col]; g = canonicalize(adata.obs[grp_col])
...
def main():
    ...
    with open(OUTFILE_TXT, "w") as fh:
        fh.write("STEP 4a -- Downstream Disease Prediction (AGE, scFoundation)\n" + "="*80 + "\n")
        for dname, fpath in DATASETS.items():
            ...
            grp_col = detect_age_col(adata.obs, fpath)
            ...
            for model_name, model_tmpl in MODEL_TEMPLATES.items():
                for strat_name, flags in STRATEGIES.items():
                    ...
                    row = {"dataset": dname, "model": model_name, "strategy": strat_name,
                           ...
                           "celltype_worst_age": ct_worst, "celltype_worst_age_acc": ct_worst_acc,
                           "random_baseline_acc": rand_acc, "n_cells": len(y_d)}
                    for grp, a in per_group.items(): row[f"per_age_disease_acc_{grp}"] = a
                    rows.append(row)
    ...
    df = pd.DataFrame(rows); df.to_csv(OUTFILE_CSV, index=False)
