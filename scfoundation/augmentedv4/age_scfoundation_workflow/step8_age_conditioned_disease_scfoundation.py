#!/usr/bin/env python3
"""STEP 8 — Sex-Conditioned Disease Prediction (AGE, scFoundation)"""
...
OUTDIR = BASE / "step8_age_conditioned_disease_scfoundation"; OUTDIR.mkdir(exist_ok=True)
...
EMB_KEY = "X_scfoundation"; DISEASE_COL = "disease"; AGE_BIN_KEY = "age_bin_10yr"
...
def train_and_evaluate(ad, dataset_name, seed=RANDOM_STATE):
    ...
    y = ad.obs.loc[valid, "disease_binary"].values; g = ad.obs.loc[valid, AGE_BIN_KEY].values
    ...
    for grp in np.unique(g_te):
        ...
        rows.append({"dataset": dataset_name, "age_bin_10yr": grp,
...
def main():
    log("="*70); log("STEP 8 -- Sex-Conditioned Disease Prediction (AGE, scFoundation)"); log("="*70)
    all_dfs = []
    ...
    worst_age_df = (per_age_df.loc[per_age_df.groupby("dataset")["accuracy"].idxmin()]
        [["dataset", "age_bin_10yr", "accuracy", "macro_f1", "delta_acc_vs_prop"]]
        .rename(columns={"age_bin_10yr": "worst_age", "accuracy": "worst_acc", "macro_f1": "worst_f1"}).reset_index(drop=True))
    underrep_df = (per_age_df[per_age_df["is_underrep"]]
        .groupby("dataset")[["accuracy", "balanced_accuracy", "macro_f1", "delta_acc_vs_prop"]].mean().reset_index())
    per_age_df.to_csv(OUTDIR / "step8_per_age_disease_prediction_scfoundation.csv", index=False)
    worst_age_df.to_csv(OUTDIR / "step8_worst_age_summary_scfoundation.csv", index=False)
    underrep_df.to_csv(OUTDIR / "step8_underrepresented_age_summary_scfoundation.csv", index=False)
    lines = ["STEP 8 -- AGE-CONDITIONED DISEASE PREDICTION (AGE, scFoundation)", "="*70, "",
             ...
             "", "="*70, "WORST-AGE PER DATASET", "="*70, worst_age_df.to_string(index=False),
             "", "="*70, f"UNDER-REPRESENTED GROUP ({UNDERREP_GROUP})", "="*70, underrep_df.to_string(index=False)]
    (OUTDIR / "step8_summary_scfoundation_age.txt").write_text("\n".join(lines))
    print("\n" + "="*70 + "\nWORST-AGE SUMMARY"); print(worst_age_df.to_string(index=False))
