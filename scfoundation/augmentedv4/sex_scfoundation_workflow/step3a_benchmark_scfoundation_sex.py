#!/usr/bin/env python3
"""STEP 3a — scIB Benchmarking (SEX, scFoundation)"""
...
OUTDIR = BASE / "benchmark_outputs_scfoundation_sex"
OUTDIR.mkdir(parents=True, exist_ok=True)

OUTPUT_BASE = "ILD_Sex_Pilot"
DATASETS = {
    "BalancedAugmented_1413Each": f"{OUTPUT_BASE}_BalancedAugmented_1413Each_SEX_scfoundation.h5ad",
    "Proportional_1999":          f"{OUTPUT_BASE}_Proportional_1999_SEX_scfoundation.h5ad",
    "BalancedUpsampled_1413Each": f"{OUTPUT_BASE}_BalancedUpsampled_1413Each_SEX_scfoundation.h5ad",
    "Downsampled_586Each":        f"{OUTPUT_BASE}_Downsampled_586Each_SEX_scfoundation.h5ad",
}

EMB_KEY   = "X_scfoundation"
LABEL_KEY = "cluster_labels"
SEX_COL_CANDIDATES  = ["sex", "Sex", "SEX"]
UNKNOWN_SEX_VALUES  = {"unknown", "na", "n/a", "not reported", "", "nan"}
MIN_CELLS_PER_GROUP = 5

def detect_sex_col(ad, fname):
    for c in SEX_COL_CANDIDATES:
        if c in ad.obs.columns: return c
    raise RuntimeError(f"No sex column in {fname}. Columns: {list(ad.obs.columns)}")
...
def canonicalize_sex(ad, col):
    ...
    small = ad.obs[col].value_counts()
    small = small[small < MIN_CELLS_PER_GROUP].index.tolist()
...
def run_scib(mode, fname):
    ...
    for grp, n in ad.obs[sex_col].value_counts().items(): print(f"     {grp}: {n}")
    ...
    ad_slim.obsp = ad.obsp; ad_slim.uns = ad.uns
    bm = Benchmarker(ad_slim, batch_key=sex_col, label_key=LABEL_KEY,
...
def main():
    ...
    print(f"\n{'='*70}\nSTEP 3a COMPLETE (SEX, scFoundation)\n\n" + df.to_string(index=False))
