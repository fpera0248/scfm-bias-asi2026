#!/usr/bin/env python3
"""STEP 3b — kNN Disease Label Propagation (SEX, scFoundation)"""
...
DATASETS = {
    "BalancedAugmented_1413Each": BASE / f"{OUTPUT_BASE}_BalancedAugmented_1413Each_SEX_scfoundation.h5ad",
    "Proportional_1999":          BASE / f"{OUTPUT_BASE}_Proportional_1999_SEX_scfoundation.h5ad",
    "BalancedUpsampled_1413Each": BASE / f"{OUTPUT_BASE}_BalancedUpsampled_1413Each_SEX_scfoundation.h5ad",
    "Downsampled_586Each":        BASE / f"{OUTPUT_BASE}_Downsampled_586Each_SEX_scfoundation.h5ad",
}
...
def propagate_labels(adata, dname):
    ...
    if SEX_COL in adata.obs.columns:
        for grp, n in adata.obs[SEX_COL].value_counts().items(): log(f"    {grp}: {n}")
    disease_vals   = adata.obs[DISEASE_COL].to_numpy()
    real_has_label = real_mask & (~pd.isna(disease_vals))
    X_real = emb[real_has_label]
    y_real = disease_vals[real_has_label].astype(str)
    nan_real = np.isnan(X_real).any(axis=1)
    X_real = X_real[~nan_real]; y_real = y_real[~nan_real]
...
def main():
    ...
    log("="*70); log("STEP 3b -- kNN Disease Label Propagation (SEX, scFoundation)"); log("="*70)
    summary_rows = []
    ...
    log(f"\n{'='*70}\nSTEP 3b COMPLETE (SEX, scFoundation)")
