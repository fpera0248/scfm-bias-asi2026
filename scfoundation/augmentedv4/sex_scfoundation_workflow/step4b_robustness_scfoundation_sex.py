#!/usr/bin/env python3
"""STEP 4b — Robustness Stress Test (SEX, scFoundation)"""
...
OUTPUT_BASE = "ILD_Sex_Pilot"; LABELED_DIR = BASE / "step3b_labeled"
DATASETS = {
...
OUTDIR = BASE / "step4b_model_robustness_tests_sex_scfoundation"; OUTDIR.mkdir(exist_ok=True)
CSV_FILE = OUTDIR / "step4b_results_sex_labeled_scfoundation.csv"
TXT_FILE = OUTDIR / "step4b_full_log_sex_labeled_scfoundation.txt"

logger = logging.getLogger("STEP4B_SEX_SF"); logger.setLevel(logging.INFO)
fmt = logging.Formatter("%(asctime)s | %(levelname)s | %(message)s")
fh = logging.FileHandler(TXT_FILE); fh.setFormatter(fmt); logger.addHandler(fh)
sh = logging.StreamHandler(); sh.setFormatter(fmt); logger.addHandler(sh)
warnings.filterwarnings("always")
logger.info("========== STEP 4b START (SEX, scFoundation) ==========")
...
for dname, path in DATASETS.items():
    ...
    y0 = ad.obs["disease_binary"].to_numpy(); g0 = ad.obs["_sex"].to_numpy()
        ...
        for strat, flags in STRATS.items():
            ...
            for mname, model in make_models().items():
                ...
                row = dict(dataset=dname, repeat=rep, model=mname, strategy=strat,
                           accuracy=float(acc), macro_f1=float(f1), worst_sex=worst, worst_sex_acc=float(wacc),
                ...
                for grp in KNOWN_GROUPS: row[f"per_sex_acc_{grp}"] = float(per.get(grp, np.nan))
                rows.append(row)
...
logger.info(f"========== STEP 4b COMPLETE (SEX, scFoundation) ==========  Runtime: {total_min:.2f} min")
