#!/usr/bin/env python3
"""STEP 4 -- External Validation Disease + Cell-Type Classification (SEX, scFoundation)"""
...
VALIDATION_FILE = BASE / "ILD_Sex_External_Validation_5000_scfoundation.h5ad"
CSV_FILE = OUTDIR / "step4_external_validation_results_sex_scfoundation.csv"
TXT_FILE = OUTDIR / "step4_external_validation_log_sex_scfoundation.txt"

EMB_KEY        = "X_scfoundation"
DISEASE_COL    = "disease"
CELLTYPE_COL   = "cell_type"
SEX_COL        = "sex"
DISEASE_GROUPS = {"male", "female"}
RANDOM_STATE   = 42
EOS_NEIGHBORS  = 5
EOS_MULTIPLIER = 0.50
AR_BINS        = 30
...
def canonicalize_sex(series): return series.astype(str).str.strip().str.lower()
...
def main():
    ...
    log("="*70); log("STEP 4 -- External Validation Classification (SEX, scFoundation)"); log("="*70)
    log("\nLoading validation set ...")
    X_val, sex_val, dis_val, ct_val = load_adata("Validation", VALIDATION_FILE)
    disease_mask = pd.Series(sex_val).isin(DISEASE_GROUPS).to_numpy()
    X_val_dis = X_val[disease_mask]; sex_val_dis = sex_val[disease_mask]; dis_val_dis = dis_val[disease_mask]
    log(f"  Disease subset: {X_val_dis.shape[0]:,} cells  Cell-type: {X_val.shape[0]:,} cells")
    rows = []
    for pname, ppath in PILOTS.items():
        log(f"\n{'='*70}\nPilot: {pname}")
        try: X_pilot, sex_pilot, dis_pilot, ct_pilot = load_adata(pname, ppath)
        ...
        X_pilot_dis = X_pilot[pilot_disease_mask]; sex_pilot_dis = sex_pilot[pilot_disease_mask]; dis_pilot_dis = dis_pilot[pilot_disease_mask]
        for model_name, model_tmpl in MODEL_TEMPLATES.items():
            for strat_name, flags in STRATEGIES.items():
                ...
                dres = evaluate_external(model_tmpl, X_pilot_dis, dis_pilot_dis, sex_pilot_dis,
                                         X_val_dis, dis_val_dis, sex_val_dis, use_ar=flags["use_ar"], use_eos=flags["use_eos"])
                cres = evaluate_external(model_tmpl, X_pilot, ct_pilot, sex_pilot,
                                         X_val, ct_val, sex_val, use_ar=flags["use_ar"], use_eos=flags["use_eos"])
                ...
                if dres:
                    row.update({"disease_accuracy": dres["accuracy"], "disease_macro_f1": dres["macro_f1"],
                                "disease_worst_sex": dres["worst_group"], "disease_worst_acc": dres["worst_acc"],
