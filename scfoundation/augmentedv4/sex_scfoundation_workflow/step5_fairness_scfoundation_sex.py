#!/usr/bin/env python3
"""STEP 5 — Fairness Stress Test (SEX, scFoundation)"""
...
OUTDIR = BASE / "step5_outputs_scfoundation_sex"; OUTDIR.mkdir(exist_ok=True)

EMB_KEY = "X_scfoundation"; GROUP_KEY = "sex"; DISEASE_COL = "disease"; SOURCE_COL = "source"
...
def strict_sex_balance_indices(ad, min_required=MIN_REQUIRED, seed=RANDOM_STATE):
    ...
    valid_groups = [grp for grp in tab[GROUP_KEY].unique()
                    if tab[tab[GROUP_KEY]==grp]["disease_binary"].nunique()==2 and tab[tab[GROUP_KEY]==grp]["count"].min() > 0]
    if len(valid_groups) < 2: raise RuntimeError(f"Not enough sex groups. Found: {valid_groups}")
...
t_start = time.time()
report_lines = ["STEP 5 -- FAIRNESS STRESS TEST (SEX, scFoundation)\n" + "="*70 + "\n"]
...
for ds_name, path in DATASETS.items():
    ...
    try:
        ...
        log(f"  Strict balance failed: {e}"); cls_strict = None; sil_strict = np.nan; min_g = 0; valid_groups = []
    ...
    if cls_strict: report_lines.append(f"Strict balanced (min={min_g}, groups={valid_groups}): acc={cls_strict['accuracy']} f1={cls_strict['macro_f1']}")
    summary_rows.append({
...
(OUTDIR / "step5_fairness_stress_test_scfoundation_sex.txt").write_text("\n".join(report_lines))
pd.DataFrame(summary_rows).to_csv(OUTDIR / "step5_summary_scfoundation_sex.csv", index=False)
lc_nonempty = [df for df in all_lc_rows if not df.empty]
if lc_nonempty: pd.concat(lc_nonempty, ignore_index=True).to_csv(OUTDIR / "step5_learning_curves_scfoundation_sex.csv", index=False)
log("Done.")
