#!/usr/bin/env python3
"""STEP 4a — Downstream Disease Prediction (SEX, scFoundation)"""
...
OUTDIR = BASE / "step4a_downstream_scfoundation_sex"
OUTDIR.mkdir(exist_ok=True)
OUTFILE_TXT = OUTDIR / "step4a_downstream_results_sex_AR_EOS_scfoundation.txt"
OUTFILE_CSV = OUTDIR / "step4a_downstream_results_sex_AR_EOS_scfoundation.csv"
...
SEX_COL_CANDIDATES = ["sex", "Sex", "SEX"]
...
def main():
    ...
    with open(OUTFILE_TXT, "w") as fh:
        fh.write("STEP 4a -- Downstream Disease Prediction (SEX, scFoundation)\n" + "="*80 + "\n")
        for dname, fpath in DATASETS.items():
            ...
            grp_col = detect_sex_col(adata.obs, fpath)
            ...
            for model_name, model_tmpl in MODEL_TEMPLATES.items():
                for strat_name, flags in STRATEGIES.items():
                    ...
                    row = {"dataset": dname, "model": model_name, "strategy": strat_name,
                           ...
                           "disease_worst_sex": worst_grp, "disease_worst_sex_acc": worst_acc,
                           "celltype_accuracy": ct_acc, "celltype_macro_f1": ct_f1,
                           "celltype_worst_sex": ct_worst, "celltype_worst_sex_acc": ct_worst_acc,
                           "random_baseline_acc": rand_acc, "n_cells": len(y_d)}
                    for grp, a in per_group.items(): row[f"per_sex_disease_acc_{grp}"] = a
                    rows.append(row)
    ...
    print("\n" + df[["dataset","model","strategy","disease_accuracy","disease_macro_f1","disease_worst_sex_acc","celltype_accuracy"]].to_string(index=False))
