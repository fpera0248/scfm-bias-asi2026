#!/usr/bin/env Rscript
# ============================================================
# STEP 0B STAGE 2: PER-GROUP scDesign3 AUGMENTATION (SEX)
# ============================================================
...
if (!BIN_LABEL %in% names(bin_sces)) {
  stop(sprintf("Bin '%s' not in stage1 (have: %s)",
...
if (length(other_anchors) == 0) {
  ...
  log_msg("WARN: no anchor cells available")
} else {
  ...
  log_msg(sprintf("Joint training: %d target + %d anchor = %d total",
                  ncol(sub_train_target), ncol(sub_train) - ncol(sub_train_target),
                  ncol(sub_train)))
}

# Simplified mu_formula: just sex + cell_type structure + library offset
build_mu_formula <- function(train_sce) {
  parts <- c("cell_type", "sex")
...
log_msg(sprintf("important_feature: %d / %d (threshold=%.2f)",
