#!/usr/bin/env python3
"""STEP 6 — Per-Sex Fairness Diagnostics (SEX, scFoundation)"""
...
OUTDIR = BASE / "step6_outputs_scfoundation_sex"; OUTDIR.mkdir(exist_ok=True)
...
EMB_KEY = "X_scfoundation"; GROUP_KEY = "sex"; DISEASE_COL = "disease"
...
def silhouette_per_sex(ad):
    ...
    for grp in sorted(ad_r.obs[GROUP_KEY].unique()):
        ...
        rows.append({"sex": grp, "silhouette": round(float(sil), 4)})
    return pd.DataFrame(rows)
def knn_mixing_per_sex(ad, k=KNN_K):
    ...
    df = pd.DataFrame({"sex": groups, "knn_mixing": mixing})
    return df.groupby("sex")["knn_mixing"].mean().reset_index()
def accuracy_per_sex(ad, seed=RANDOM_STATE):
    ...
    return {grp: round(float(accuracy_score(y_te[g_te==grp], preds[g_te==grp])), 4) for grp in np.unique(g_te) if (g_te==grp).sum() >= 5}
def ari_nmi_per_sex(ad):
    ...
    for grp in sorted(ad_r.obs[GROUP_KEY].unique()):
        ...
        rows.append({"sex": grp, "ari": round(float(adjusted_rand_score(y_true, y_pred)), 4),
                     "nmi": round(float(normalized_mutual_info_score(y_true, y_pred)), 4)})
    return pd.DataFrame(rows)

def main():
    log("="*70); log("STEP 6 -- Per-Sex Fairness Diagnostics (SEX, scFoundation)"); log("="*70)
    datasets = {}
    ...
    for label, ad in datasets.items():
        ...
        sil_df = silhouette_per_sex(ad); mix_df = knn_mixing_per_sex(ad)
        acc_d = accuracy_per_sex(ad); acc_df = pd.DataFrame([{"sex": g, "accuracy": a} for g, a in acc_d.items()])
        ari_df = ari_nmi_per_sex(ad)
        df = sil_df.copy()
        for other in [mix_df, acc_df, ari_df]:
            if not other.empty: df = df.merge(other, on="sex", how="outer") if not df.empty else other
    ...
    pivot = df_all.pivot_table(index="sex", columns="dataset", values=metric_cols)
    ...
    pivot.to_csv(OUTDIR / "step6_per_sex_diagnostics_scfoundation.csv")
    delta_cols = [c for c in pivot.columns if "delta" in str(c[1])]
    lines = ["STEP 6 -- PER-SEX FAIRNESS DIAGNOSTICS (SEX, scFoundation)", "="*70, "",
             "KNN MIXING: random expected ~0.50 with 2 groups.",
    ...
    (OUTDIR / "step6_per_sex_diagnostics_report_scfoundation.txt").write_text("\n".join(lines))
    log("STEP 6 COMPLETE")
