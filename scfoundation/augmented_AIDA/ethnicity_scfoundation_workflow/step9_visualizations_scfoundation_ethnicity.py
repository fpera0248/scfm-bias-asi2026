#!/usr/bin/env python3
"""STEP 9 — Results Visualization (ETHNICITY, scFoundation)"""
...
DATASET_ORDER  = ["Proportional_2497", "BalancedAugmented_2143Each", "BalancedUpsampled_2143Each", "Downsampled_48Each"]
...
UMAP_DATASETS = {
    "Proportional_2497":          "AIDA_Ethnicity_Pilot_Proportional_2497_ETHNICITY_scfoundation",
    "BalancedAugmented_2143Each": "AIDA_Ethnicity_Pilot_BalancedAugmented_2143Each_ETHNICITY_scfoundation",
    "BalancedUpsampled_2143Each": "AIDA_Ethnicity_Pilot_BalancedUpsampled_2143Each_ETHNICITY_scfoundation",
    "Downsampled_48Each":         "AIDA_Ethnicity_Pilot_Downsampled_48Each_ETHNICITY_scfoundation",
}
UMAP_DISPLAY = {
...
def fig1_scib():
    ...
    for metric, (_, short, title, subtitle) in SCIB_METRIC_MAP.items():
        ...
        _single_bar_figure(
            title=f"Fig 1 — scIB Embedding Quality (scFoundation ETHNICITY)\n{title}",
...
def fig2_step4a():
    ...
    for metric, panel_title, ylabel, fname in panels:
        ...
        fig.suptitle(f"Fig 2 — Disease Prediction Strategies (scFoundation ETHNICITY)\n{panel_title}",
...
def fig3_step4b():
    ...
    for ds in datasets:
        ...
        fig, ax = plt.subplots(figsize=(14, 7))
        fig.suptitle(f"Fig 3 — Model Robustness (scFoundation ETHNICITY)\nDataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
        if data_vals:
...
def fig4_learning_curves():
    ...
    for ds in datasets:
        sub = df[df["dataset"] == ds]
        fig, ax = plt.subplots(figsize=(11, 7))
        fig.suptitle(f"Fig 4 — Learning Curves (scFoundation ETHNICITY)\nDataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig5_step6():
    ...
    for eth in df.index.tolist():
        ...
        for metric in metrics:
            ...
            fig.suptitle(
                f"Fig 5 — Per-Ethnicity Fairness Deltas (scFoundation ETHNICITY)\n"
...
def fig6_step7():
    ...
    for metric in metrics:
        ...
        fig.suptitle(f"Fig 6 — Representation Quality (scFoundation ETHNICITY)\n{title}",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig7_step8():
    ...
    fig.suptitle("Fig 7A — Ethnicity-Conditioned Disease Prediction (scFoundation ETHNICITY)\n"
    ...
    if not worst.empty:
        ...
        fig.suptitle("Fig 7B — Worst-Performing Ethnicity Group Accuracy (scFoundation ETHNICITY)",
                     fontsize=17, fontweight="bold", y=1.02)
    ...
    if "delta_acc_vs_prop" in nat_rows.columns and not nat_rows.empty:
        ...
        fig.suptitle(f"Fig 7C — {UNDERREP_GROUP.title()} Accuracy Delta vs Proportional (scFoundation ETHNICITY)",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig8_summary_heatmap():
    ...
    fig.suptitle("Fig 8 — Summary Heatmap (scFoundation ETHNICITY)\n"
...
def main():
    print("\nSTEP 9 — Visualization Pipeline (ETHNICITY, scFoundation)")
