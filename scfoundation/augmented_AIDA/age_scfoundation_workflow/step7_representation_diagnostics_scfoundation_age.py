#!/usr/bin/env python3
"""STEP 7 — Representation Quality & Fairness Diagnostics (AGE, scFoundation)"""
...
FILES = {
    "Proportional_2498":          LABELED_DIR / f"{OUTPUT_BASE}_Proportional_2498_AGE_scfoundation.h5ad",
    "BalancedAugmented_747Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedAugmented_747Each_AGE_scfoundation.h5ad",
    "BalancedUpsampled_747Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedUpsampled_747Each_AGE_scfoundation.h5ad",
    "Downsampled_230Each":        LABELED_DIR / f"{OUTPUT_BASE}_Downsampled_230Each_AGE_scfoundation.h5ad",
}
EMB_KEY = "X_scfoundation"; AGE_BIN_KEY = "age_bin_10yr"; CELL_KEY = "cell_type"
...
def within_celltype_mixing(ad):
    ...
    for ct, sub_idx in ad.obs.groupby(CELL_KEY, observed=False).groups.items():
        ...
        emb = np.array(ad.obsm[EMB_KEY], dtype=np.float32)[pos]; ages = ad.obs.iloc[pos][AGE_BIN_KEY].astype(str).values
        nn = NearestNeighbors(n_neighbors=KNN_K + 1).fit(emb); neigh = nn.kneighbors(return_distance=False)[:, 1:]
        mixing = np.mean(ages[neigh] != ages[:, None], axis=1)
        df = pd.DataFrame({AGE_BIN_KEY: ages, "mixing": mixing})
...
def celltype_linear_probe(ad):
    X = np.array(ad.obsm[EMB_KEY], dtype=np.float32); y = ad.obs[CELL_KEY].astype(str).values; g = ad.obs[AGE_BIN_KEY].astype(str).values
    ...
    return pd.Series({grp: round(float(f1_score(sub["y_true"], sub["y_pred"], average="macro")), 4)
                      for grp in np.unique(g) for sub in [df[df[AGE_BIN_KEY] == grp]] if sub["y_true"].nunique() >= 2})

def main():
    log("="*70); log("STEP 7 -- Representation Quality & Fairness Diagnostics (AGE, scFoundation)"); log("="*70)
    datasets = {}
    for label, path in FILES.items():
        if not path.exists(): log(f"  Skipping {label}"); continue
        datasets[label] = load_dataset(label, path)
    if "Proportional_2498" not in datasets: raise RuntimeError("Proportional_2498 required as reference baseline.")
    all_results = []
    for label, ad in datasets.items():
        ...
        df = df.reset_index().rename(columns={"index": AGE_BIN_KEY}); df["dataset"] = label; all_results.append(df)
        log(f"  Purity: {purity.to_dict()}"); log(f"  Mixing: {mixing.to_dict()}"); log(f"  ProbeF1: {probe.to_dict()}")
    df_all = pd.concat(all_results, ignore_index=True)
    pivot = df_all.pivot_table(index=AGE_BIN_KEY, columns="dataset",
        values=["celltype_purity", "within_ct_age_mixing", "celltype_macroF1"], observed=False)
    ...
    df_all.to_csv(OUTDIR / "step7_per_age_diagnostics_scfoundation.csv", index=False)
    delta_cols = [c for c in pivot.columns if "delta" in str(c[1])]
    lines = ["STEP 7 -- REPRESENTATION QUALITY & FAIRNESS DIAGNOSTICS (AGE, scFoundation)", "="*70, "",
             "NOTE: With 7 age groups, random expected kNN mixing is ~0.857.",
             "", "="*70, "FULL PIVOT TABLE", "="*70, pivot.to_string(),
             "", "="*70, "DELTA SUMMARY (vs Proportional_2498)", "="*70,
             pivot[delta_cols].sort_index().to_string() if delta_cols else "No delta columns produced."]
    (OUTDIR / "step7_summary_scfoundation_age.txt").write_text("\n".join(lines))
    log("STEP 7 COMPLETE")
