#!/usr/bin/env python3
"""
STEP 8 — Age-Conditioned Disease Prediction (CRC AGE, scFoundation)
...
OUTDIR      = BASE / "step8_age_conditioned_disease_scfoundation"   # [AGE 6]
...
def main():
    log("=" * 70)
    log("STEP 8 -- Age-Conditioned Disease Prediction (CRC AGE, scFoundation)")  # [AGE]
    log("=" * 70)
    ...
    out_per_age = OUTDIR / "step8_per_age_disease_prediction_scfoundation.csv"
    per_age_df.to_csv(out_per_age, index=False)
    log(f"\nPer-age CSV -> {out_per_age}")

    out_worst = OUTDIR / "step8_worst_age_bin_summary_scfoundation.csv"
    worst_age_df.to_csv(out_worst, index=False)
    log(f"Worst-age CSV -> {out_worst}")

    out_underrep = OUTDIR / "step8_underrepresented_age_summary_scfoundation.csv"
    underrep_df.to_csv(out_underrep, index=False)
    log(f"Under-rep CSV -> {out_underrep}")

    out_txt = OUTDIR / "step8_summary_age_scfoundation.txt"
    lines = [
        "STEP 8 -- AGE-CONDITIONED DISEASE PREDICTION (CRC AGE, scFoundation)",
        "=" * 70, "",
