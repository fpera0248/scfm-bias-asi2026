#!/usr/bin/env python3
"""STEP 3a — scIB Benchmarking (CRC AGE, scFoundation)"""
import os
import scanpy as sc
import pandas as pd
import numpy as np
...
DATASETS = {
    "Proportional_2498":          f"{OUTPUT_BASE}_Proportional_2498_AGE_scfoundation.h5ad",
    "BalancedAugmented_650Each": f"{OUTPUT_BASE}_BalancedAugmented_650Each_AGE_scfoundation.h5ad",
    "BalancedUpsampled_650Each": f"{OUTPUT_BASE}_BalancedUpsampled_650Each_AGE_scfoundation.h5ad",
    "Downsampled_124Each":         f"{OUTPUT_BASE}_Downsampled_124Each_AGE_scfoundation.h5ad",
}
...
def run_scib(mode, fname):
    ...
    bm = Benchmarker(ad_slim, batch_key=age_col, label_key=LABEL_KEY,
        ...
        batch_correction_metrics=BatchCorrection( ilisi_knn=True, kbet_per_label=True, graph_connectivity=False, pcr_comparison=True),
        n_jobs=-1)
    try: bm.benchmark()
        ...
        if "arpack" in str(e).lower():
            bm = Benchmarker(ad_slim, batch_key=age_col, label_key=LABEL_KEY,
                embedding_obsm_keys=[EMB_KEY],
                bio_conservation_metrics=BioConservation(nmi_ari_cluster_labels_kmeans=True, clisi_knn=True, isolated_labels=False, silhouette_label=False),
                batch_correction_metrics=BatchCorrection( ilisi_knn=True, kbet_per_label=False, graph_connectivity=False, pcr_comparison=True),
                n_jobs=-1)
            bm.benchmark()
        else: raise
