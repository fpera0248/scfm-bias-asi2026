#!/usr/bin/env python3
"""STEP 4a — Downstream Disease Prediction (CRC AGE, scFoundation) with AR + EOS"""
...
OUTDIR = BASE / "step4a_downstream_scfoundation_age"; OUTDIR.mkdir(exist_ok=True)
...
OUTPUT_BASE = "CRC_Age_Pilot"; LABELED_DIR = BASE / "step3b_labeled"
DATASETS = {
...
def canonicalize_age(series): return series.astype(str).str.strip().str.lower()
def make_binary(x):
    if pd.isna(x): return np.nan
    return "normal" if str(x).lower().strip() == "normal" else "disease"

def extract_features(adata, label_col, grp_col):
    if EMB_KEY not in adata.obsm: raise RuntimeError(f"Missing embedding '{EMB_KEY}'")
    X = np.array(adata.obsm[EMB_KEY], dtype=np.float32)
    y = adata.obs[label_col]; g = canonicalize_age(adata.obs[grp_col])
...
def calculate_adaptive_weights_pdf(X_train, bins=AR_BINS):
    ...
    for k in range(d):
        ...
        dim_weights[:, k] = 1.0 / ((1.0 - ALPHA) * hist[bin_idx] * (edges[1]-edges[0]) + ALPHA / n)
...
def apply_eos(X_train, y_train, g_train, target_group):
    ...
    synth = np.vstack(synth); y_new = rng.choice(y_min, size=len(synth)); g_new = np.repeat(target_group, len(synth))
...
def evaluate(model_template, X, y, g, use_ar=False, use_eos=False):
    X_tr, X_te, y_tr, y_te, g_tr, g_te = train_test_split(X, y, g, test_size=TEST_SIZE, stratify=safe_stratify(y), random_state=RANDOM_STATE)
...
def main():
    ...
    with open(OUTFILE_TXT, "w") as fh:
        fh.write("STEP 4a -- Downstream Disease Prediction (CRC AGE, scFoundation) with AR + EOS\n" + "="*80 + "\n")
        for dname, fpath in DATASETS.items():
            if not fpath.exists(): log(f"  Skipping {dname} -- file not found"); continue
            log(f"Loading {dname}")
            adata = sc.read_h5ad(fpath); adata.obs_names_make_unique()
            grp_col = detect_age_col(adata.obs, fpath)
            X, y_raw, g = extract_features(adata, DISEASE_COL, grp_col)
            y_bin = pd.Series([make_binary(v) for v in y_raw]).map({"normal": 0, "disease": 1})
            keep  = ~pd.isna(y_bin)
            ...
            fh.write(f"\n{'='*80}\nDataset: {dname}\n  Cells: {len(y_d):,}\n  Random baseline: acc={rand_acc:.3f}\n")
            for model_name, model_tmpl in MODEL_TEMPLATES.items():
                for strat_name, flags in STRATEGIES.items():
                    ...
                    acc, f1, worst_grp, worst_acc, per_group = evaluate(model_tmpl, X_d, y_d, g_d, **flags)
                    ct_acc, ct_f1, ct_worst, ct_worst_acc, ct_per = evaluate(model_tmpl, X_ct, y_ct, g_ct, **flags)
                    row = {"dataset": dname, "model": model_name, "strategy": strat_name,
                           "disease_accuracy": acc, "disease_macro_f1": f1,
                           "disease_worst_age_bin": worst_grp, "disease_worst_age_bin_acc": worst_acc,
                           "celltype_accuracy": ct_acc, "celltype_macro_f1": ct_f1,
                           "celltype_worst_age_bin": ct_worst, "celltype_worst_age_bin_acc": ct_worst_acc,
                    ...
                    rows.append(row)
                    fh.write(f"  {model_name:12s} | {strat_name:8s} | acc={acc:.3f} f1={f1:.3f} | worst={worst_grp}:{worst_acc:.3f} | ct={ct_acc:.3f}\n")
                    log(f"    -> acc={acc:.3f} f1={f1:.3f} | worst={worst_grp}:{worst_acc:.3f} | ct={ct_acc:.3f}")
    df = pd.DataFrame(rows); df.to_csv(OUTFILE_CSV, index=False)
    log("STEP 4a COMPLETE"); log(f"   Results -> {OUTFILE_CSV}")
    print("\n" + df[["dataset","model","strategy","disease_accuracy","disease_macro_f1","disease_worst_age_bin_acc","celltype_accuracy"]].to_string(index=False))
