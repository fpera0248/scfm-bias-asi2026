#!/usr/bin/env python3
"""
STEP 7 — Representation Quality & Fairness Diagnostics (CRC AGE, scFoundation)
...
OUTDIR      = BASE / "step7_representation_diagnostics_age_scfoundation"   # [AGE 1]
...
def main():
    log("=" * 70)
    log("STEP 7 -- Representation Quality & Fairness Diagnostics (CRC AGE, scFoundation)")
    log("=" * 70)
    ...
    out_csv = OUTDIR / "step7_per_age_diagnostics_scfoundation.csv"
    df_all.to_csv(out_csv, index=False)
    log(f"\nCSV saved -> {out_csv}")

    out_txt = OUTDIR / "step7_summary_age_scfoundation.txt"
    lines = [
        "STEP 7 -- REPRESENTATION QUALITY & FAIRNESS DIAGNOSTICS (CRC AGE, scFoundation)",
        "=" * 70, "",
