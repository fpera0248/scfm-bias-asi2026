#!/usr/bin/env python3
"""STEP 9 — Results Visualization (CRC ETHNICITY, scFoundation)"""
...
STEP4B_DIR = BASE / "step4b_model_robustness_tests_ethnicity_scfoundation"
STEP5_DIR  = BASE / "step5_outputs_scfoundation_ethnicity"
STEP6_DIR  = BASE / "step6_outputs_scfoundation_ethnicity"
STEP7_DIR  = BASE / "step7_representation_diagnostics_scfoundation_ethnicity"
STEP8_DIR  = BASE / "step8_eth_conditioned_disease_scfoundation"
...
STRATEGY_PALETTE = {
    "Baseline": "#4C72B0", "AR": "#937860", "EOS": "#55A868", "AR+EOS": "#C44E52",
    "EOS_adv": "#55A868", "AR+EOS_adv": "#C44E52",
}
...
ETH_COLORS = {
    ...
    "hispanic or latin": "#388E3C", "native american": "#D32F2F", "african american": "#7B1FA2",
}
CANONICAL_ETH_MAP = {
    "asian": "Asian", "european american": "European American",
    "hispanic or latin": "Hispanic or Latin", "native american": "Native American",
...
ETH_OBS_COL  = "self_reported_ethnicity"
UNDERREP_GROUP = "african american"
DATASET_ORDER  = ["Proportional_2497", "BalancedAugmented_1880Each", "BalancedUpsampled_1880Each", "Downsampled_48Each"]
...
UMAP_DATASETS = {
    "Proportional_2497":          "CRC_Eth_Pilot_Proportional_2497_ETHNICITY_scfoundation",
    "BalancedAugmented_1880Each": "CRC_Eth_Pilot_BalancedAugmented_1880Each_ETHNICITY_scfoundation",
    "BalancedUpsampled_1880Each": "CRC_Eth_Pilot_BalancedUpsampled_1880Each_ETHNICITY_scfoundation",
    "Downsampled_48Each":         "CRC_Eth_Pilot_Downsampled_48Each_ETHNICITY_scfoundation",
}
UMAP_DISPLAY = {
...
def _single_bar_figure(title, subtitle, xlabel, ylabel, datasets, values, colors, fname,
                       annotate=True, ylim_pad=1.35, hline=None, hline_label=None):
...
def _canonicalize_eth_obs(adata):
    if ETH_OBS_COL in adata.obs.columns:
        raw = adata.obs[ETH_OBS_COL].astype(str).str.strip().str.lower()
        adata.obs[ETH_OBS_COL] = raw.map(CANONICAL_ETH_MAP).fillna("European American")
    return adata
...
def generate_individual_umaps():
    ...
    for ds_key, ds_stem in UMAP_DATASETS.items():
        ...
        fpath   = BASE / f"{ds_stem}.h5ad"
        if not fpath.exists(): print(f"  Skipping UMAP for {ds_key}"); continue
        print(f"\n  [scFoundation] {ds_key}")
        ...
        if ETH_OBS_COL in adata.obs.columns:
            ...
            _umap_scatter(ax, coords, adata.obs[ETH_OBS_COL].astype(str).values,
                          ETH_PALETTE_UMAP, pt_size,
                          f"{display}\nColored by Ethnicity (scFoundation Embedding)",
                          legend_outside=True)
            fig.subplots_adjust(left=0.07, right=0.62, top=0.88, bottom=0.10)
            save(fig, f"umap_scfoundation_{_safe(display)}_ethnicity.png")
        ...
        for col in ("cell_type", "celltype"):
            if col in adata.obs.columns:
                ...
                _umap_scatter(ax, coords, adata.obs[col].astype(str).values,
                              ct_palette, pt_size,
                              f"{display}\nColored by Cell Type (scFoundation Embedding)",
                              legend_outside=True)
                fig.subplots_adjust(left=0.07, right=0.60, top=0.88, bottom=0.10)
                save(fig, f"umap_scfoundation_{_safe(display)}_celltype.png")
                break
        ...
        if "source" in adata.obs.columns:
            ...
            plt.tight_layout()
            save(fig, f"umap_scfoundation_{_safe(display)}_source.png")
        del adata
    print("\n  UMAPs complete.")


def load_scib():
    path = STEP3A_DIR / "benchmark_summary_all_modes.csv"
...
def fig1_scib():
    ...
    for metric, (_, short, title, subtitle) in SCIB_METRIC_MAP.items():
        ...
        _single_bar_figure(
            title=f"Fig 1 — scIB Embedding Quality (CRC scFoundation ETHNICITY)\n{title}",
...
def fig2_step4a():
    path = STEP4A_DIR / "step4a_downstream_results_ethnicity_AR_EOS_scfoundation.csv"
    if not path.exists(): print("  Fig 2: Step 4a CSV not found — skipping"); return
    df = pd.read_csv(path); ref = "Proportional_2497"
    ...
    panels = [
        ...
        ("disease_worst_ethnicity_acc", "Worst-Ethnicity Disease Prediction Accuracy", "Accuracy", "step4a_fig2_worst_eth_accuracy.png"),
    ]
    for metric, panel_title, ylabel, fname in panels:
        fig, ax = plt.subplots(figsize=(13, 7))
        fig.suptitle(f"Fig 2 — Disease Prediction Strategies (CRC scFoundation ETHNICITY)\n{panel_title}",
        ...
        ax.set_ylabel(ylabel, fontsize=14, labelpad=10)
        ax.set_xticks(x)
        ax.set_xticklabels([STRATEGY_DESCRIPTIONS.get(s, s) for s in strategies], fontsize=12, ha="center")
...
def fig3_step4b():
    path = STEP4B_DIR / "step4b_results_ethnicity_labeled_scfoundation.csv"
    ...
    for ds in datasets:
        ...
        positions, data_vals, colors_list = [], [], []; pos = 0; group_centers, group_labels = [], []
        for strat in strats:
            group_start = pos
            for model in models:
        ...
        fig, ax = plt.subplots(figsize=(14, 7))
        fig.suptitle(f"Fig 3 — Model Robustness (CRC scFoundation ETHNICITY)\nDataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
        if data_vals:
...
def fig4_learning_curves():
    path = STEP5_DIR / "step5_learning_curves_scfoundation_ethnicity.csv"
    ...
    for ds in datasets:
        sub = df[df["dataset"] == ds]
        fig, ax = plt.subplots(figsize=(11, 7))
        fig.suptitle(f"Fig 4 — Learning Curves (CRC scFoundation ETHNICITY)\nDataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
        ...
        ax.yaxis.grid(True, alpha=0.3); ax.xaxis.grid(True, alpha=0.3); ax.set_ylim(0.30, 1.05)
        plt.tight_layout(pad=2.5)
        save(fig, f"step5_fig4_learning_curves_{_safe(ds)}.png")


def fig5_step6():
    path = STEP6_DIR / "step6_per_ethnicity_diagnostics_scfoundation.csv"
    ...
    for eth in df.index.tolist():
        ...
        for metric in metrics:
            ...
            fig.suptitle(
                f"Fig 5 — Per-Ethnicity Fairness Deltas (CRC scFoundation ETHNICITY)\n"
            ...
            ax.set_xticks(range(len(labels))); ax.set_xticklabels(labels, fontsize=13, ha="center")
...
def fig6_step7():
    path = STEP7_DIR / "step7_per_ethnicity_diagnostics_scfoundation.csv"
    ...
    GROUP_COL = "self_reported_ethnicity" if "self_reported_ethnicity" in df.columns else None
    if GROUP_COL is None: print("  Fig 6: No ethnicity column — skipping"); return
    ...
    groups = sorted(df[GROUP_COL].unique()); width = 0.18; x = np.arange(len(datasets_in))
    for metric in metrics:
        title, ylabel, fname_stem = metric_info[metric]
        fig, ax = plt.subplots(figsize=(max(14, len(datasets_in) * 3.5), 8))
        fig.suptitle(f"Fig 6 — Representation Quality (CRC scFoundation ETHNICITY)\n{title}",
                     fontsize=17, fontweight="bold", y=1.02)
        ...
        for i, grp in enumerate(groups):
            ...
            color  = ETH_COLORS.get(grp, "#888888")
        ...
        ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
        if "mixing" in metric:
            expected = round(1 - 1 / len(groups), 2)
            ax.axhspan(expected - 0.1, expected + 0.1, alpha=0.08, color="green", zorder=1,
                       label=f"Expected random ({expected})")
            ax.set_ylim(0, 1.08)
        else:
            ax.set_ylim(0, min(max_overall * 1.40, 1.08))
        ax.legend(title="Ethnicity Group", fontsize=11, title_fontsize=12,
...
def fig7_step8():
    per_eth_path = STEP8_DIR / "step8_per_ethnicity_disease_prediction_scfoundation.csv"
    worst_path   = STEP8_DIR / "step8_worst_ethnicity_summary_scfoundation.csv"
    if not per_eth_path.exists(): print("  Fig 7: Step 8 CSV not found — skipping"); return
    ...
    width    = 0.18; x = np.arange(len(datasets))

    fig, ax = plt.subplots(figsize=(max(14, len(datasets) * 3.5), 8))
    fig.suptitle("Fig 7A — Ethnicity-Conditioned Disease Prediction (CRC scFoundation ETHNICITY)\n"
                 "Per-Ethnicity Accuracy by Dataset Variant",
                 fontsize=17, fontweight="bold", y=1.02)
    for i, grp in enumerate(sorted(groups)):
        sub    = per_eth[per_eth["ethnicity"] == grp].set_index("dataset").reindex(datasets)
        offset = (i - len(groups) / 2 + 0.5) * width
        color  = ETH_COLORS.get(grp, "#888888")
    ...
    ax.axhline(0.5, color="gray", linestyle="--", linewidth=1.2, alpha=0.5)
    ax.yaxis.grid(True, alpha=0.3, zorder=0)
    ax.legend(title="Ethnicity Group", fontsize=11, title_fontsize=12, loc="upper right", ncol=2)
    plt.tight_layout(pad=2.5)
    ...
    if not worst.empty:
        ...
        fig.suptitle("Fig 7B — Worst-Performing Ethnicity Group Accuracy (CRC scFoundation ETHNICITY)",
                     fontsize=17, fontweight="bold", y=1.02)
        ...
        ax.set_ylim(0.0, 1.12); ax.yaxis.grid(True, alpha=0.3, zorder=0)
        plt.tight_layout(pad=2.5)
        save(fig, "step8_fig7_worst_ethnicity_accuracy.png")

    nat_rows = per_eth[per_eth["ethnicity"] == UNDERREP_GROUP].copy()
    if "delta_acc_vs_prop" in nat_rows.columns and not nat_rows.empty:
        nat_plot = nat_rows[nat_rows["dataset"] != "Proportional_2497"].copy()
        nat_plot = nat_plot.set_index("dataset").reindex(
            [d for d in DATASET_ORDER if d != "Proportional_2497"]).reset_index()
        fig, ax = plt.subplots(figsize=(max(9, len(nat_plot) * 2.6), 7))
        fig.suptitle(f"Fig 7C — {UNDERREP_GROUP.title()} Accuracy Delta vs Proportional (CRC scFoundation ETHNICITY)",
                     fontsize=17, fontweight="bold", y=1.02)
        if nat_plot["delta_acc_vs_prop"].notna().any():
            colors = ["#DD8452" if v >= 0 else "#C44E52" for v in nat_plot["delta_acc_vs_prop"].fillna(0)]
            bars = ax.bar(range(len(nat_plot)), nat_plot["delta_acc_vs_prop"].fillna(0),
                          color=colors, alpha=0.85, edgecolor="white", width=0.55, zorder=3)
            ax.axhline(0, color="black", linewidth=2.0, zorder=4)
            ax.set_xticks(range(len(nat_plot)))
            ax.set_xticklabels([ds_label(d).replace("\n", " ") for d in nat_plot["dataset"]],
                               fontsize=13, ha="center")
            for bar, (_, row) in zip(bars, nat_plot.iterrows()):
        ...
        ax.yaxis.grid(True, alpha=0.3, zorder=0)
        plt.tight_layout(pad=2.5)
        save(fig, "step8_fig7_african_american_delta.png")
...
def fig8_summary_heatmap():
    ...
    p = STEP4A_DIR / "step4a_downstream_results_ethnicity_AR_EOS_scfoundation.csv"
    if p.exists():
        df4  = pd.read_csv(p)
    ...
    p = STEP8_DIR / "step8_worst_ethnicity_summary_scfoundation.csv"
    if p.exists():
        for _, row in pd.read_csv(p).iterrows():
            rows.append({"dataset": row["dataset"], "Worst-Eth (S8)": row.get("worst_acc", np.nan)})
    p = STEP8_DIR / "step8_per_ethnicity_disease_prediction_scfoundation.csv"
    if p.exists():
        df8b = pd.read_csv(p)
        nat  = df8b[df8b["ethnicity"].str.lower().str.strip() == UNDERREP_GROUP]
    ...
    fig.suptitle("Fig 8 — Summary Heatmap (CRC scFoundation ETHNICITY)\n"
...
def main():
    print("\nSTEP 9 — Visualization Pipeline (CRC ETHNICITY, scFoundation)")
