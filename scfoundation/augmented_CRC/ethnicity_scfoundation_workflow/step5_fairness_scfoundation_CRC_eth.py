#!/usr/bin/env python3
"""STEP 5 — Fairness Stress Test (CRC ETHNICITY, scFoundation)"""

import scanpy as sc, numpy as np, pandas as pd, pathlib, time, warnings
...
EMB_KEY = "X_scfoundation"; GROUP_KEY = "self_reported_ethnicity"; DISEASE_COL = "disease"; SOURCE_COL = "source"
OUTPUT_BASE = "CRC_Eth_Pilot"
DATASETS = {
    "BalancedAugmented_1880Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedAugmented_1880Each_labeled_scfoundation.h5ad",
    "Proportional_2497":          LABELED_DIR / f"{OUTPUT_BASE}_Proportional_2497_labeled_scfoundation.h5ad",
    "BalancedUpsampled_1880Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedUpsampled_1880Each_labeled_scfoundation.h5ad",
    "Downsampled_48Each":         LABELED_DIR / f"{OUTPUT_BASE}_Downsampled_48Each_labeled_scfoundation.h5ad",
}
RANDOM_STATE = 42; TEST_SIZE = 0.20; MIN_REQUIRED = 10; LC_CV_FOLDS = 5; LC_N_POINTS = 8
...
def extract_scaled(ad):
    ...
    if bad.any(): X, y, g, syn = X[~bad], y[~bad], g[~bad], syn[~bad]
    valid = pd.notna(y) & pd.notna(g); Xv = X[valid.values]
...
def evaluate_classifier(X, y, g, seed=RANDOM_STATE):
    ...
    counts = pd.Series(y).value_counts(); stratify = y if (counts >= 2).all() else None
    ...
    per_eth = {eth: accuracy_score(y_te[g_te==eth], preds[g_te==eth]) for eth in np.unique(g_te) if (g_te==eth).sum() >= 5}
    worst = min(per_eth, key=per_eth.get) if per_eth else "NA"
    return {"accuracy": round(float(accuracy_score(y_te, preds)), 4),
            ...
            "worst_ethnicity": str(worst), "worst_eth_accuracy": round(float(per_eth.get(worst, np.nan)), 4) if worst != "NA" else np.nan,
...
def strict_ethnicity_balance_indices(ad, min_required=MIN_REQUIRED, seed=RANDOM_STATE):
    rng = np.random.default_rng(seed); obs = ad.obs.copy(); obs = obs[pd.notna(obs["disease_binary"])]
    ...
    valid_eths = [eth for eth in tab[GROUP_KEY].unique()
                  if tab[tab[GROUP_KEY]==eth]["disease_binary"].nunique()==2 and tab[tab[GROUP_KEY]==eth]["count"].min() > 0]
    if len(valid_eths) < 2: raise RuntimeError(f"Not enough ethnicity groups. Found: {valid_eths}")
    obs2 = obs[obs[GROUP_KEY].isin(valid_eths)].copy()
    groups = obs2.groupby([GROUP_KEY, "disease_binary"], observed=False); min_g = int(groups.size().min())
    if min_g < min_required: raise RuntimeError(f"min_group_size too small: {min_g}")
    selected = []
    for (_, _), idx in groups.groups.items(): selected.extend(rng.choice(list(idx), size=min_g, replace=False))
...
def compute_learning_curves(X, y, dataset_name):
    ...
    train_sizes = np.linspace(0.10, 1.0, LC_N_POINTS); results = []
    for model_name, estimator in [
        ...
        try:
            ts, tr_scores, val_scores = learning_curve(estimator=estimator, X=X, y=y, train_sizes=train_sizes,
            ...
            for i, n in enumerate(ts):
                results.append({"dataset": dataset_name, "model": model_name, "n_train": int(n),
                                 "train_f1_mean": round(float(tr_scores[i].mean()), 4), "train_f1_std": round(float(tr_scores[i].std()), 4),
                                 "val_f1_mean": round(float(val_scores[i].mean()), 4), "val_f1_std": round(float(val_scores[i].std()), 4)})
        except Exception as e: log(f"  WARNING: LC failed for {model_name}: {e}")
    return pd.DataFrame(results)

t_start = time.time()
report_lines = ["STEP 5 -- FAIRNESS STRESS TEST (CRC ETHNICITY, scFoundation)\n" + "="*70 + "\n"]
all_lc_rows = []; summary_rows = []
for ds_name, path in DATASETS.items():
    if not path.exists(): log(f"  Skipping {ds_name}"); continue
    log(f"\n{'='*60}\nDataset: {ds_name}")
    ad = load_dataset(path)
    log(f"  Cells: {ad.n_obs:,}  Ethnicity dist: {ad.obs[GROUP_KEY].value_counts().to_dict()}")
    X, y, g, syn = extract_scaled(ad)
    sil = safe_silhouette(X, y, syn); cls_full = evaluate_classifier(X, y, g)
    if cls_full is None: log(f"  Degenerate split -- skipping"); continue
    try:
        sel_idx, min_g, valid_eths = strict_ethnicity_balance_indices(ad)
        ad_strict = ad[sel_idx].copy(); Xs, ys, gs, syns = extract_scaled(ad_strict)
        sil_strict = safe_silhouette(Xs, ys, syns); cls_strict = evaluate_classifier(Xs, ys, gs)
    except RuntimeError as e:
        log(f"  Strict balance failed: {e}"); cls_strict = None; sil_strict = np.nan; min_g = 0; valid_eths = []
    lc_df = compute_learning_curves(X, y, ds_name); all_lc_rows.append(lc_df)
    report_lines += [f"\nDATASET: {ds_name}\n" + "-"*60,
                     f"Cells: {len(y):,}  Synthetic: {syn.sum():,}  Silhouette: {sil}",
                     f"Accuracy: {cls_full['accuracy']}  F1: {cls_full['macro_f1']}  Worst eth: {cls_full['worst_ethnicity']} ({cls_full['worst_eth_accuracy']})"]
    if cls_strict: report_lines.append(f"Strict balanced (min={min_g}, groups={valid_eths}): acc={cls_strict['accuracy']} f1={cls_strict['macro_f1']}")
    summary_rows.append({
        "dataset": ds_name, "n_labeled": len(y), "n_synthetic": int(syn.sum()), "silhouette": sil,
        "accuracy": cls_full["accuracy"], "balanced_accuracy": cls_full["balanced_accuracy"], "macro_f1": cls_full["macro_f1"],
...
(OUTDIR / "step5_fairness_stress_test_scfoundation_ethnicity.txt").write_text("\n".join(report_lines))
pd.DataFrame(summary_rows).to_csv(OUTDIR / "step5_summary_scfoundation_ethnicity.csv", index=False)
lc_nonempty = [df for df in all_lc_rows if not df.empty]
if lc_nonempty: pd.concat(lc_nonempty, ignore_index=True).to_csv(OUTDIR / "step5_learning_curves_scfoundation_ethnicity.csv", index=False)
log("Done.")
