#!/usr/bin/env python3
"""
Generates steps 3a-9 for CRC ethnicity scFoundation workflow.
Run on Oscar: python3 generate_CRC_eth_scFoundation_steps3to9.py
"""
import os, subprocess

BASE_ILD = "/oscar/home/fperalta/data/fperalta/scfoundation/augmentedv4/ethnicity_scfoundation_workflow"
BASE_CRC = "/oscar/home/fperalta/data/fperalta/scfoundation/augmented_CRC/ethnicity_scfoundation_workflow"
...
def adapt(src: str) -> str:
    return (src
        ...
        # file suffix: ILD uses _ETHNICITY_scfoundation, CRC uses _ETH_scfoundation
        .replace("_ETHNICITY_scfoundation.h5ad", "_ETH_scfoundation.h5ad")
        # validation file
        .replace(
            'VALIDATION_FILE = BASE / "ILD_Ethnicity_External_Validation_12500_scfoundation.h5ad"',
            f'VALIDATION_FILE = BASE / "{VAL_FILE}_ETH_scfoundation.h5ad"\n'
        ...
        # step9 UMAP stems
        .replace(
            '"ILD_Ethnicity_Pilot_Proportional_2497_ETHNICITY_scfoundation"',
            '"CRC_Eth_Pilot_Proportional_1998_ETH_scfoundation"')
        .replace(
            '"ILD_Ethnicity_Pilot_BalancedAugmented_2143Each_ETHNICITY_scfoundation"',
            '"CRC_Eth_Pilot_BalancedAugmented_1504Each_ETH_scfoundation"')
        .replace(
            '"ILD_Ethnicity_Pilot_BalancedUpsampled_2143Each_ETHNICITY_scfoundation"',
            '"CRC_Eth_Pilot_BalancedUpsampled_1504Each_ETH_scfoundation"')
        .replace(
            '"ILD_Ethnicity_Pilot_Downsampled_48Each_ETHNICITY_scfoundation"',
            '"CRC_Eth_Pilot_Downsampled_90Each_ETH_scfoundation"')
        # step9 UMAP display
        .replace('"Proportional (2,497 cells — real only)"',
                 '"Proportional (1,998 cells — real only)"')
        .replace('"Balanced Augmented (2,143/group — scDesign3)"',
                 '"Balanced Augmented (1,504/group — scDesign3)"')
        .replace('"Balanced Upsampled (2,143/group — real only)"',
                 '"Balanced Upsampled (1,504/group — real only)"')
        .replace('"Downsampled (48/group — real only)"',
                 '"Downsampled (90/group — real only)"')
        # underrep figure labels
        .replace(
            '"Fig 7C — native american.title() Accuracy Delta vs Proportional (scFoundation ETHNICITY)"',
            '"Fig 7C — African American Accuracy Delta vs Proportional (CRC scFoundation ETHNICITY)"')
        .replace('"step8_fig7_native_american_delta.png"',
...
script_names = {
    "step3a": "step3a_benchmark_scfoundation_ethnicity.py",
    "step3b": "step3b_label_propagation_scfoundation_ethnicity.py",
    "step4":  "step4_external_validation_scfoundation_ethnicity.py",
    "step4a": "step4a_downstream_scfoundation_ethnicity.py",
    "step4b": "step4b_robustness_scfoundation_ethnicity.py",
    "step5":  "step5_fairness_scfoundation_ethnicity.py",
    "step6":  "step6_per_ethnicity_diagnostics_scfoundation.py",
    "step7":  "step7_representation_diagnostics_scfoundation_ethnicity.py",
    "step8":  "step8_eth_conditioned_disease_scfoundation.py",
    "step9":  "step9_visualizations_scfoundation_ethnicity.py",
}

out_names = {
    "step3a": "step3a_benchmark_scfoundation_CRC_eth.py",
    "step3b": "step3b_label_propagation_scfoundation_CRC_eth.py",
    "step3d": "step3d_verify_validation_scfoundation_CRC_eth.py",
    "step4":  "step4_external_validation_scfoundation_CRC_eth.py",
    "step4a": "step4a_downstream_scfoundation_CRC_eth.py",
    "step4b": "step4b_robustness_scfoundation_CRC_eth.py",
    "step5":  "step5_fairness_scfoundation_CRC_eth.py",
    "step6":  "step6_per_ethnicity_diagnostics_scfoundation_CRC.py",
    "step7":  "step7_representation_diagnostics_scfoundation_CRC_eth.py",
    "step8":  "step8_eth_conditioned_disease_scfoundation_CRC.py",
    "step9":  "step9_visualizations_scfoundation_CRC_eth.py",
}
...
candidates = [
    BASE / "{VAL_FILE}_ETH_scfoundation.h5ad",
...
for tag, script, partition, mem, tlimit in slurm_specs:
    slurm = f"""#!/bin/bash
#SBATCH --job-name=CRC_eth_scf_{tag}
#SBATCH --partition={partition}
#SBATCH --nodes=1
#SBATCH --cpus-per-task=8
#SBATCH --mem={mem}
#SBATCH --time={tlimit}
#SBATCH --output={BASE_CRC}/logs/CRC_eth_scf_{tag}_%j.out
#SBATCH --error={BASE_CRC}/logs/CRC_eth_scf_{tag}_%j.err
...
echo "Exit: $?"
"""
    with open(f"{BASE_CRC}/{tag}_CRC_eth_scf.slurm", "w") as f:
        f.write(slurm)
    print(f"Written: {tag}_CRC_eth_scf.slurm")
...
"""
with open(f"{BASE_CRC}/submit_CRC_eth_scFoundation_steps.sh", "w") as f:
    f.write(submit)
print(f"\nWritten: submit_CRC_eth_scFoundation_steps.sh")
...
for tag in ["step3b", "step8"]:
    ...
    result = subprocess.run(
        ["grep", "-n", r"OUTPUT_BASE\|DATASETS\|UNDERREP\|BASE =\|Proportional\|1504\|1998\|90Each\|8572\|KNOWN_GROUP\|DISEASE_GROUPS\|VALIDATION\|_ETH_"],
        stdin=open(path), capture_output=True, text=True)
    print(f"\n--- {out_names[tag]} ---")
    print(result.stdout[:800])

print("\nAll done. Manually run steps 3a, 3b, 3d, then:")
print(f"  bash {BASE_CRC}/submit_CRC_eth_scFoundation_steps.sh")
