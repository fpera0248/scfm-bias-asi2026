#!/usr/bin/env python3
"""STEP 6 — Per-Ethnicity Fairness Diagnostics (CRC ETHNICITY, scFoundation)"""
...
OUTDIR = BASE / "step6_outputs_scfoundation_ethnicity"; OUTDIR.mkdir(exist_ok=True)
...
EMB_KEY = "X_scfoundation"; GROUP_KEY = "self_reported_ethnicity"; DISEASE_COL = "disease"
CELL_KEY = "cell_type"; SOURCE_COL = "source"; KNN_K = 15; RANDOM_STATE = 42
...
def silhouette_per_ethnicity(ad):
    ...
    if ad_r.n_obs < 50: return pd.DataFrame()
    X = np.array(ad_r.obsm[EMB_KEY], dtype=np.float32); y = ad_r.obs["disease_binary"].values; rows = []
...
def knn_mixing_per_ethnicity(ad, k=KNN_K):
    X = np.array(ad.obsm[EMB_KEY], dtype=np.float32); groups = ad.obs[GROUP_KEY].astype(str).values
    nn = NearestNeighbors(n_neighbors=k + 1).fit(X); neigh = nn.kneighbors(return_distance=False)[:, 1:]
...
def accuracy_per_ethnicity(ad, seed=RANDOM_STATE):
    ...
    y = ad.obs.loc[valid, "disease_binary"].values; g = ad.obs.loc[valid, GROUP_KEY].values
    if len(np.unique(y)) < 2: return {}
    counts = pd.Series(y).value_counts(); stratify = y if (counts >= 2).all() else None
    X = StandardScaler().fit_transform(X)
    X_tr, X_te, y_tr, y_te, g_tr, g_te = train_test_split(X, y, g, test_size=0.2, stratify=stratify, random_state=seed)
    clf = LogisticRegression(max_iter=5000, solver="saga", class_weight="balanced", n_jobs=-1)
    clf.fit(X_tr, y_tr); preds = clf.predict(X_te)
    return {eth: round(float(accuracy_score(y_te[g_te==eth], preds[g_te==eth])), 4) for eth in np.unique(g_te) if (g_te==eth).sum() >= 5}
def ari_nmi_per_ethnicity(ad):
    if CELL_KEY not in ad.obs.columns: return pd.DataFrame()
    ad_r = ad[(~ad.obs["is_synthetic"])].copy()
    if ad_r.n_obs < 50: return pd.DataFrame()
    ...
    for eth in sorted(ad_r.obs[GROUP_KEY].unique()):
        ...
        y_true = sub.obs[CELL_KEY].astype(str).values; y_pred = sub.obs["leiden"].astype(str).values
        if len(np.unique(y_true)) < 2 or len(np.unique(y_pred)) < 2: continue
        rows.append({"ethnicity": eth, "ari": round(float(adjusted_rand_score(y_true, y_pred)), 4),
                     "nmi": round(float(normalized_mutual_info_score(y_true, y_pred)), 4)})
    return pd.DataFrame(rows)

def main():
    log("="*70); log("STEP 6 -- Per-Ethnicity Fairness Diagnostics (CRC ETHNICITY, scFoundation)"); log("="*70)
    datasets = {}
    for label, path in FILES.items():
        if not path.exists(): log(f"  Skipping {label}"); continue
        datasets[label] = load_dataset(label, path)
    if "Proportional_2497" not in datasets: raise RuntimeError("Proportional_2497 required as reference baseline.")
    all_results = []
    for label, ad in datasets.items():
        log(f"\n>> Computing metrics for {label}")
        sil_df = silhouette_per_ethnicity(ad); mix_df = knn_mixing_per_ethnicity(ad)
        acc_d = accuracy_per_ethnicity(ad); acc_df = pd.DataFrame([{"ethnicity": e, "accuracy": a} for e, a in acc_d.items()])
        ari_df = ari_nmi_per_ethnicity(ad)
        df = sil_df.copy()
        for other in [mix_df, acc_df, ari_df]:
            if not other.empty: df = df.merge(other, on="ethnicity", how="outer") if not df.empty else other
        if not df.empty: df["dataset"] = label; all_results.append(df)
    if not all_results: raise RuntimeError("No metrics produced.")
    ...
    pivot.to_csv(OUTDIR / "step6_per_ethnicity_diagnostics_scfoundation.csv")
    delta_cols = [c for c in pivot.columns if "delta" in str(c[1])]
    lines = ["STEP 6 -- PER-ETHNICITY FAIRNESS DIAGNOSTICS (CRC ETHNICITY, scFoundation)", "="*70, "",
             "KNN MIXING: random expected ~0.80 with 5 groups.",
             "", "="*70, "FULL PIVOT TABLE", "="*70, pivot.to_string(),
             "", "="*70, "DELTA SUMMARY (vs Proportional_2497)", "="*70,
             pivot[delta_cols].sort_index().to_string() if delta_cols else "No delta columns produced."]
    (OUTDIR / "step6_per_ethnicity_diagnostics_report_scfoundation.txt").write_text("\n".join(lines))
    log("STEP 6 COMPLETE")
