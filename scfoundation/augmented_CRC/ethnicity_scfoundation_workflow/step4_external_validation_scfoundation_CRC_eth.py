#!/usr/bin/env python3
"""STEP 4 -- External Validation Disease + Cell-Type Classification (CRC ETHNICITY, scFoundation)"""
...
OUTDIR      = BASE / "step4_external_validation_scfoundation_ethnicity"
...
VALIDATION_FILE = BASE / "CRC_Eth_External_Validation_8572_ETH_scfoundation.h5ad"
if not VALIDATION_FILE.exists():
    VALIDATION_FILE = BASE / "step2a_embeddings" / "CRC_Eth_External_Validation_8572_scfoundation.h5ad"
CSV_FILE = OUTDIR / "step4_external_validation_results_ethnicity_scfoundation.csv"
TXT_FILE = OUTDIR / "step4_external_validation_log_ethnicity_scfoundation.txt"
...
def canonicalize_ethnicity(series): return series.astype(str).str.strip().str.lower()
...
def main():
    ...
    log("="*70); log("STEP 4 -- External Validation Classification (CRC ETHNICITY, scFoundation)"); log("="*70)
    log("\nLoading validation set ...")
    X_val, eth_val, dis_val, ct_val = load_adata("Validation", VALIDATION_FILE)
    disease_mask = pd.Series(eth_val).isin(DISEASE_GROUPS).to_numpy()
    X_val_dis = X_val[disease_mask]; eth_val_dis = eth_val[disease_mask]; dis_val_dis = dis_val[disease_mask]
    log(f"  Disease subset: {X_val_dis.shape[0]:,} cells  Cell-type: {X_val.shape[0]:,} cells")
    rows = []
    for pname, ppath in PILOTS.items():
        log(f"\n{'='*70}\nPilot: {pname}")
        try: X_pilot, eth_pilot, dis_pilot, ct_pilot = load_adata(pname, ppath)
        ...
        X_pilot_dis = X_pilot[pilot_disease_mask]; eth_pilot_dis = eth_pilot[pilot_disease_mask]; dis_pilot_dis = dis_pilot[pilot_disease_mask]
        for model_name, model_tmpl in MODEL_TEMPLATES.items():
            for strat_name, flags in STRATEGIES.items():
                ...
                dres = evaluate_external(model_tmpl, X_pilot_dis, dis_pilot_dis, eth_pilot_dis,
                                         X_val_dis, dis_val_dis, eth_val_dis, use_ar=flags["use_ar"], use_eos=flags["use_eos"])
                cres = evaluate_external(model_tmpl, X_pilot, ct_pilot, eth_pilot,
                                         X_val, ct_val, eth_val, use_ar=flags["use_ar"], use_eos=flags["use_eos"])
                ...
                if dres:
                    row.update({"disease_accuracy": dres["accuracy"], "disease_macro_f1": dres["macro_f1"],
                                "disease_worst_eth": dres["worst_group"], "disease_worst_acc": dres["worst_acc"],
                ...
                if cres:
                    row.update({"celltype_accuracy": cres["accuracy"], "celltype_macro_f1": cres["macro_f1"],
                                "celltype_worst_eth": cres["worst_group"], "celltype_worst_acc": cres["worst_acc"]})
                    log(f"    celltype: acc={cres['accuracy']:.3f} f1={cres['macro_f1']:.3f}")
                rows.append(row)
    df = pd.DataFrame(rows); df.to_csv(CSV_FILE, index=False)
    log(f"\n{'='*70}\nSTEP 4 COMPLETE (CRC ETHNICITY, scFoundation)")
