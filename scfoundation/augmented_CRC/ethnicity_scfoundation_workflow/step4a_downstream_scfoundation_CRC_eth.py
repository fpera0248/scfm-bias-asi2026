#!/usr/bin/env python3
"""STEP 4a — Downstream Disease Prediction (CRC ETHNICITY, scFoundation)"""
...
BASE   = Path("/data/scfoundation/augmented_CRC/ethnicity_scfoundation_workflow")
...
OUTFILE_CSV = OUTDIR / "step4a_downstream_results_ethnicity_AR_EOS_scfoundation.csv"

OUTPUT_BASE = "CRC_Eth_Pilot"; LABELED_DIR = BASE / "step3b_labeled"
DATASETS = {
...
EMB_KEY = "X_scfoundation"; DISEASE_COL = "disease"; CELLTYPE_COL = "cell_type"
ETHNICITY_COL_CANDIDATES = ["self_reported_ethnicity", "ethnicity", "Ethnicity", "ETHNICITY"]
RANDOM_STATE = 42; TEST_SIZE = 0.20; EOS_NEIGHBORS = 5; EOS_MULTIPLIER = 0.50; AR_BINS = 30
...
def extract_features(adata, label_col, grp_col):
    X = np.array(adata.obsm[EMB_KEY], dtype=np.float32); y = adata.obs[label_col]; g = canonicalize(adata.obs[grp_col])
...
def random_baseline(y):
    rng = np.random.default_rng(RANDOM_STATE); preds = rng.choice(np.unique(y), size=len(y))
    return accuracy_score(y, preds), f1_score(y, preds, average="macro")
def safe_stratify(y):
    counts = pd.Series(y).value_counts(); return None if (counts < 2).any() else y
def calculate_adaptive_weights_pdf(X_train, bins=AR_BINS):
    ...
    for k in range(d):
        ...
        prob_s = (1.0 - ALPHA) * hist[bin_idx] * (edges[1]-edges[0]) + ALPHA / n; dim_weights[:, k] = 1.0 / prob_s
...
def apply_eos(X_train, y_train, g_train, target_group):
    ...
    for _ in range(int(len(X_min) * EOS_MULTIPLIER)):
        ...
        j = rng.choice(enemy); synth.append(X_min[i0] + rng.uniform(0., 1.) * (X_min[i0] - X_train[j]))
    if not synth: return X_train, y_train, g_train
    synth = np.vstack(synth)
    return np.vstack([X_train, synth]), np.concatenate([y_train, rng.choice(y_min, len(synth))]), np.concatenate([g_train, np.repeat(target_group, len(synth))])
def evaluate(model_template, X, y, g, use_ar=False, use_eos=False):
    stratify = safe_stratify(y)
    X_tr, X_te, y_tr, y_te, g_tr, g_te = train_test_split(X, y, g, test_size=TEST_SIZE, stratify=stratify, random_state=RANDOM_STATE)
    scaler = StandardScaler(); X_tr = scaler.fit_transform(X_tr); X_te = scaler.transform(X_te)
    if use_eos:
        minority = pd.Series(g_tr).value_counts().idxmin(); X_tr, y_tr, g_tr = apply_eos(X_tr, y_tr, g_tr, minority)
    ...
    preds = model.predict(X_te); acc = accuracy_score(y_te, preds); f1 = f1_score(y_te, preds, average="macro")
    per_group = {grp: accuracy_score(y_te[g_te==grp], preds[g_te==grp]) for grp in np.unique(g_te) if (g_te==grp).sum() > 0}
    worst_grp = min(per_group, key=per_group.get) if per_group else "n/a"
    return acc, f1, worst_grp, per_group.get(worst_grp, np.nan), per_group

def main():
    warnings.filterwarnings("ignore"); rows = []
    with open(OUTFILE_TXT, "w") as fh:
        fh.write("STEP 4a -- Downstream Disease Prediction (CRC ETHNICITY, scFoundation)\n" + "="*80 + "\n")
        for dname, fpath in DATASETS.items():
            if not fpath.exists(): log(f"  Skipping {dname}"); continue
            log(f"Loading {dname}"); adata = sc.read_h5ad(fpath); adata.obs_names_make_unique()
            grp_col = detect_ethnicity_col(adata.obs, fpath)
            X, y_raw, g = extract_features(adata, DISEASE_COL, grp_col)
            y_bin = pd.Series([make_binary_disease(v) for v in y_raw]).map({"normal": 0, "disease": 1})
            keep = ~pd.isna(y_bin)
            ...
            fh.write(f"\n{'='*80}\nDataset: {dname}  Cells: {len(y_d):,}  Random: acc={rand_acc:.3f} f1={rand_f1:.3f}\n")
            for model_name, model_tmpl in MODEL_TEMPLATES.items():
                for strat_name, flags in STRATEGIES.items():
                    log(f"  {model_name} | {strat_name}")
                    acc, f1, worst_grp, worst_acc, per_group = evaluate(model_tmpl, X_d, y_d, g_d, use_ar=flags["use_ar"], use_eos=flags["use_eos"])
                    ct_acc, ct_f1, ct_worst, ct_worst_acc, ct_per = evaluate(model_tmpl, X_ct, y_ct, g_ct, use_ar=flags["use_ar"], use_eos=flags["use_eos"])
                    row = {"dataset": dname, "model": model_name, "strategy": strat_name,
                           ...
                           "celltype_worst_ethnicity": ct_worst, "celltype_worst_eth_acc": ct_worst_acc,
                           "random_baseline_acc": rand_acc, "n_cells": len(y_d)}
                    for grp, a in per_group.items(): row[f"per_eth_disease_acc_{grp}"] = a
                    rows.append(row)
    ...
    df = pd.DataFrame(rows); df.to_csv(OUTFILE_CSV, index=False); log("STEP 4a COMPLETE")
