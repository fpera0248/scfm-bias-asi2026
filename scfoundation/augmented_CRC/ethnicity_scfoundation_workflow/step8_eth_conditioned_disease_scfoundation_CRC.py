#!/usr/bin/env python3
"""STEP 8 — Ethnicity-Conditioned Disease Prediction (CRC ETHNICITY, scFoundation)"""
...
OUTDIR = BASE / "step8_eth_conditioned_disease_scfoundation"; OUTDIR.mkdir(exist_ok=True)
...
EMB_KEY = "X_scfoundation"; DISEASE_COL = "disease"; ETH_KEY = "self_reported_ethnicity"
UNDERREP_GROUP = "african american"; MIN_SAMPLES = 2; TEST_SIZE = 0.20; RANDOM_STATE = 42
...
def train_and_evaluate(ad, dataset_name, seed=RANDOM_STATE):
    ...
    y = ad.obs.loc[valid, "disease_binary"].values; g = ad.obs.loc[valid, ETH_KEY].values
    ...
    small_underrep = {eth for eth in all_groups if (g == eth).sum() < SMALL_GROUP_THRESHOLD and (g_te == eth).sum() < MIN_SAMPLES}
    ...
    for eth in np.unique(g_te):
        ...
        rows.append({"dataset": dataset_name, "ethnicity": eth,
                     ...
                     "n_test_cells": int(m.sum()), "is_underrep": eth == UNDERREP_GROUP, "test_boosted": eth in to_boost})
...
def main():
    log("="*70); log("STEP 8 -- Ethnicity-Conditioned Disease Prediction (CRC ETHNICITY, scFoundation)"); log("="*70)
    all_dfs = []
    ...
    worst_eth_df = (per_eth_df.loc[per_eth_df.groupby("dataset")["accuracy"].idxmin()]
        [["dataset", "ethnicity", "accuracy", "macro_f1", "delta_acc_vs_prop"]]
        .rename(columns={"ethnicity": "worst_ethnicity", "accuracy": "worst_acc", "macro_f1": "worst_f1"}).reset_index(drop=True))
    underrep_df = (per_eth_df[per_eth_df["is_underrep"]]
        .groupby("dataset")[["accuracy", "balanced_accuracy", "macro_f1", "delta_acc_vs_prop"]].mean().reset_index())
    per_eth_df.to_csv(OUTDIR / "step8_per_ethnicity_disease_prediction_scfoundation.csv", index=False)
    worst_eth_df.to_csv(OUTDIR / "step8_worst_ethnicity_summary_scfoundation.csv", index=False)
    underrep_df.to_csv(OUTDIR / "step8_underrepresented_ethnicity_summary_scfoundation.csv", index=False)
    lines = ["STEP 8 -- ETHNICITY-CONDITIONED DISEASE PREDICTION (CRC ETHNICITY, scFoundation)", "="*70, "",
             ...
             per_eth_df[["dataset","ethnicity","accuracy","balanced_accuracy","macro_f1","delta_acc_vs_prop","n_test_cells"]].to_string(index=False),
             "", "="*70, "WORST-ETHNICITY PER DATASET", "="*70, worst_eth_df.to_string(index=False),
             "", "="*70, f"UNDER-REPRESENTED GROUP ({UNDERREP_GROUP})", "="*70, underrep_df.to_string(index=False)]
    (OUTDIR / "step8_summary_scfoundation_ethnicity.txt").write_text("\n".join(lines))
    print("\n" + "="*70 + "\nWORST-ETHNICITY SUMMARY"); print(worst_eth_df.to_string(index=False))
