...
if want scdesign3_env; then
  ...
  conda run -n scdesign3_env Rscript -e '
    ...
    # commits). NOTE: with option A (shipping the paper BalancedAugmented pilots), the driver
    # overwrites the generated pilot with the paper file, so the exact scDesign3 version no
    # longer changes the reproduced result; we still pin it so a from-scratch regeneration
    # stays close to the paper config. This guard form is the one that builds cleanly.
