#!/usr/bin/env python3
"""
STEP 3b — kNN Disease Label Propagation (CRC ETHNICITY, scGPT)
...
Changes vs Geneformer ethnicity version:
  ...
  [SCGPT 5] Output     -> step3b_labeled/, _labeled_scgpt.h5ad suffix
"""
...
BASE   = Path("/oscar/home/fperalta/data/fperalta/scGPT/augmented_CRC/ethnicity_scGPT_workflow")
...
DATASETS = {                                                     # [SCGPT 4]
    "BalancedAugmented_1880Each": INDIR / f"{OUTPUT_BASE}_BalancedAugmented_1880Each_ETH_scgpt.h5ad",
    "Proportional_2497":          INDIR / f"{OUTPUT_BASE}_Proportional_2497_ETH_scgpt.h5ad",
    "BalancedUpsampled_1880Each": INDIR / f"{OUTPUT_BASE}_BalancedUpsampled_1880Each_ETH_scgpt.h5ad",
    "Downsampled_48Each":         INDIR / f"{OUTPUT_BASE}_Downsampled_48Each_ETH_scgpt.h5ad",
}
...
def make_output_path(dname: str) -> Path:
    return OUTDIR / f"{OUTPUT_BASE}_{dname}_labeled_scgpt.h5ad"  # [SCGPT 5]
...
def main():
    ...
    log("=" * 70)
    log("STEP 3b -- kNN Disease Label Propagation (CRC ETHNICITY, scGPT)")
    log("=" * 70)
    log(f"KNN_K       = {KNN_K}")
    ...
    log(f"\n{'='*70}")
    log("STEP 3b COMPLETE (CRC ETHNICITY, scGPT)")
