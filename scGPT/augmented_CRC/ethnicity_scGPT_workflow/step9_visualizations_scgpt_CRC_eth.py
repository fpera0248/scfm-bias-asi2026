#!/usr/bin/env python3
"""
STEP 9 — Results Visualization (CRC ETHNICITY, scGPT)
...
DATASET_ORDER = [
    "Proportional_2497", "BalancedAugmented_1880Each",
    "BalancedUpsampled_1880Each", "Downsampled_48Each",
]
...
UMAP_DATASETS = {                                                        # [SCGPT 6]
    "Proportional_2497":          "CRC_Eth_Pilot_Proportional_2497_scgpt",
    "BalancedAugmented_1880Each": "CRC_Eth_Pilot_BalancedAugmented_1880Each_scgpt",
    "BalancedUpsampled_1880Each": "CRC_Eth_Pilot_BalancedUpsampled_1880Each_scgpt",
    "Downsampled_48Each":         "CRC_Eth_Pilot_Downsampled_48Each_scgpt",
}

UMAP_DISPLAY = {
    "Proportional_2497":          "Proportional (2,497 cells — real only)",
    "BalancedAugmented_1880Each": "Balanced Augmented (2,143/group — scDesign3)",
    "BalancedUpsampled_1880Each": "Balanced Upsampled (2,143/group — real only)",
...
def fig1_scib():
    ...
    for metric, (_, short, title, subtitle) in SCIB_METRIC_MAP.items():
        ...
        _single_bar_figure(
            title=f"Fig 1 — scIB Embedding Quality (CRC scGPT)\n{title}",
...
def fig2_step4a():
    ...
    for metric, panel_title, ylabel, fname in panels:
        ...
        fig.suptitle(f"Fig 2 — Disease Prediction Strategies (CRC scGPT)\n{panel_title}",
...
def fig3_step4b():
    ...
    for ds in datasets:
        ...
        fig, ax = plt.subplots(figsize=(14, 7))
        fig.suptitle(f"Fig 3 — Model Robustness (CRC scGPT)\nDataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
        if data_vals:
...
def fig4_learning_curves():
    ...
    for ds in datasets:
        ...
        fig.suptitle(f"Fig 4 — Learning Curves (CRC scGPT)\nDataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig5_step6():
    ...
    for eth in df.index.tolist():
        ...
        for metric in metrics:
            ...
            fig.suptitle(f"Fig 5 — Per-Ethnicity Fairness Deltas (CRC scGPT)\nGroup: {eth.title()}   Metric: {metric_titles.get(metric, metric)}",
...
def fig6_step7():
    ...
    for metric in metrics:
        ...
        fig.suptitle(f"Fig 6 — Representation Quality (CRC scGPT)\n{title}", fontsize=17, fontweight="bold", y=1.02)
...
def fig7_step8():
    ...
    fig.suptitle("Fig 7A — Ethnicity-Conditioned Disease Prediction (CRC scGPT)\nPer-Ethnicity Accuracy by Dataset Variant",
                 fontsize=17, fontweight="bold", y=1.02)
    ...
    if not worst.empty:
        ...
        fig.suptitle("Fig 7B — Worst-Performing Ethnicity Group Accuracy (CRC scGPT)",
                     fontsize=17, fontweight="bold", y=1.02)
    ...
    if "delta_acc_vs_prop" in na_rows.columns and not na_rows.empty:
        ...
        fig.suptitle("Fig 7C — African American Accuracy Delta vs Proportional (CRC scGPT)",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig8_summary_heatmap():
    ...
    fig.suptitle("Fig 8 — Summary Heatmap (CRC scGPT)\nGreen = best   Red = worst   Raw values shown",
...
def main():
    print("\nSTEP 9 — Visualization Pipeline (CRC ETHNICITY, scGPT)")
