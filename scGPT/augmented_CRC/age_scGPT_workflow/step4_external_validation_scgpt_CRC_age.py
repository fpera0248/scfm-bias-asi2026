#!/usr/bin/env python3
"""STEP 4 — External Validation Disease + Cell-Type Classification (CRC AGE, scGPT)"""
...
VALIDATION_FILE = BASE / "step2a_embeddings" / "CRC_Age_External_Validation_9402_scgpt.h5ad"
...
def calculate_ar_weights(X_train, bins=AR_BINS):
    ...
    for k in range(d):
        ...
        prob_s   = (1.0 - ALPHA) * hist[bin_idx] * bin_width + ALPHA / n
...
def apply_eos(X_train, y_train, g_train, target_group):
    ...
    rng = np.random.default_rng(RANDOM_STATE); n_new = int(len(X_min) * EOS_MULTIPLIER)
    synth = []
    ...
    return (np.vstack([X_train, synth]),
            np.concatenate([y_train, rng.choice(y_min, size=len(synth))]),
...
def evaluate_external(model_template, X_train, y_train, g_train, X_test, y_test, g_test, use_ar=False, use_eos=False):
    ...
    return dict(accuracy=acc, macro_f1=f1, per_group=per_group,
                worst_group=worst_grp, worst_acc=worst_acc, n_train=len(y_tr), n_test=len(y_test))

def main():
    warnings.filterwarnings("ignore")
    log("=" * 70); log("STEP 4 -- External Validation Classification (CRC AGE, scGPT)"); log("=" * 70)
    ...
    for pname, ppath in PILOTS.items():
        ...
        for model_name, model_tmpl in MODEL_TEMPLATES.items():
            for strat_name, flags in STRATEGIES.items():
                ...
                else: log(f"    disease: SKIPPED (single-class training)")
                if cres is not None:
                    row.update({"celltype_accuracy": cres["accuracy"], "celltype_macro_f1": cres["macro_f1"],
                                ...
                                "celltype_n_train": cres["n_train"], "celltype_n_test": cres["n_test"]})
                ...
                else: log(f"    celltype: SKIPPED (single-class training)")
                rows.append(row)
    df = pd.DataFrame(rows); df.to_csv(CSV_FILE, index=False)
    log(f"\n{'='*70}"); log(f"STEP 4 COMPLETE (CRC AGE, scGPT)"); log(f"  Results -> {CSV_FILE.name}")
