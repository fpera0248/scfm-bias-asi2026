#!/usr/bin/env python3
"""STEP 4a — Downstream Disease Prediction (CRC AGE, scGPT) with AR + EOS"""
...
EMB_KEY = "X_scGPT"; DISEASE_COL = "disease"; CELLTYPE_COL = "cell_type"
...
def safe_stratify(y):
    ...
    return None if (counts < 2).any() else y
...
def apply_eos(X_train, y_train, g_train, target_group):
    ...
    for _ in range(int(len(X_min) * EOS_MULTIPLIER)):
        ...
        j = rng.choice(enemy); R = rng.uniform(0.0, 1.0)
...
def main():
    ...
    with open(OUTFILE_TXT, "w") as fh:
        fh.write("STEP 4a -- Downstream Disease Prediction (CRC AGE, scGPT) with AR + EOS\n" + "="*80 + "\n")
