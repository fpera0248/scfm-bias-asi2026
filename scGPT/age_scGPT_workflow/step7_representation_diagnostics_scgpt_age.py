#!/usr/bin/env python3
"""
STEP 7 — Representation Quality & Fairness Diagnostics (AGE, scGPT)
...
Changes vs ethnicity scGPT version:
  ...
  [SEX 4] Reference    -> Proportional_2495
"""
...
def within_celltype_mixing(ad):
    ...
    for ct, sub_idx in ad.obs.groupby(CELL_KEY, observed=False).groups.items():
        ...
        ages   = ad.obs.iloc[pos][AGE_BIN_KEY].astype(str).values
        nn     = NearestNeighbors(n_neighbors=KNN_K + 1).fit(emb)
        neigh  = nn.kneighbors(return_distance=False)[:, 1:]
...
def celltype_linear_probe(ad):
    ...
    return pd.Series({grp: round(float(f1_score(sub["y_true"], sub["y_pred"], average="macro")), 4)
                      ...
                      for sub in [df[df[AGE_BIN_KEY] == grp]]
                      if sub["y_true"].nunique() >= 2})

def main():
    log("="*70)
    log("STEP 7 -- Representation Quality & Fairness Diagnostics (AGE, scGPT)")
    log("="*70)
    ...
    if "Proportional_2495" not in datasets:
        raise RuntimeError("Proportional_2495 required as reference baseline.")
    ...
    pivot = df_all.pivot_table(
        index=AGE_BIN_KEY, columns="dataset",
    ...
    df_all.to_csv(OUTDIR / "step7_per_age_diagnostics_scgpt.csv", index=False)
    ...
    lines = [
        ...
        pivot.to_string(), "", "="*70, "DELTA SUMMARY (vs Proportional_2495)", "="*70,
        pivot[delta_cols].sort_index().to_string() if delta_cols else "No delta columns produced.",
    ]
    (OUTDIR / "step7_summary_scgpt_age.txt").write_text("\n".join(lines))
    log("STEP 7 COMPLETE")
