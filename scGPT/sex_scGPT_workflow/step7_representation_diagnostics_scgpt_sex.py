#!/usr/bin/env python3
"""
STEP 7 — Representation Quality & Fairness Diagnostics (SEX, scGPT)
...
OUTDIR      = BASE / "step7_representation_diagnostics_scgpt_sex"
...
def celltype_linear_probe(ad):
    ...
    return pd.Series({grp: round(float(f1_score(sub["y_true"], sub["y_pred"], average="macro")), 4)
                      ...
                      for sub in [df[df[SEX_KEY] == grp]]
                      if sub["y_true"].nunique() >= 2})

def main():
    log("="*70)
    log("STEP 7 -- Representation Quality & Fairness Diagnostics (SEX, scGPT)")
    log("="*70)
    ...
    df_all.to_csv(OUTDIR / "step7_per_sex_diagnostics_scgpt.csv", index=False)
    ...
    (OUTDIR / "step7_summary_scgpt_sex.txt").write_text("\n".join(lines))
    log("STEP 7 COMPLETE")
