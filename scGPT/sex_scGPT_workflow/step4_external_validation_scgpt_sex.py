#!/usr/bin/env python3
"""
STEP 4 -- External Validation Disease + Cell-Type Classification (SEX, scGPT)

Changes vs ethnicity scGPT version:
  [SEX 1] BASE/OUTDIR    -> sex_scGPT_workflow
  [SEX 2] EMB_KEY        -> X_scGPT
  [SEX 3] PILOTS         -> sex filenames
  [SEX 4] VALIDATION     -> ILD_Sex_External_Validation_5000_scgpt.h5ad
  [SEX 5] GROUP_COL      -> "sex"
...
VALIDATION_FILE = BASE / "step2a_embeddings" / "ILD_Sex_External_Validation_5000_scgpt.h5ad"

CSV_FILE = OUTDIR / "step4_external_validation_results_sex_scgpt.csv"
TXT_FILE = OUTDIR / "step4_external_validation_log_sex_scgpt.txt"
...
def main():
    ...
    log("="*70)
    log("STEP 4 -- External Validation Classification (SEX, scGPT)")
    log("="*70)
    ...
    for pname, ppath in PILOTS.items():
        log(f"\n{'='*70}\nPilot: {pname}")
        try:
            X_pilot, sex_pilot, dis_pilot, ct_pilot = load_adata(pname, ppath)
