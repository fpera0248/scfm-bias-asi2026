#!/usr/bin/env python3
"""
STEP 9 — Results Visualization (SEX, scGPT)
...
Changes vs ethnicity scGPT version:
  ...
  [SEX 4] Step dirs     -> sex scgpt output dirs
  [SEX 5] GROUP_KEY     -> "sex"
...
STEP4A_DIR = BASE / "step4a_downstream_scgpt_sex"
STEP4B_DIR = BASE / "step4b_model_robustness_tests_sex_scgpt"
STEP5_DIR  = BASE / "step5_outputs_scgpt_sex"
STEP6_DIR  = BASE / "step6_outputs_scgpt_sex"
STEP7_DIR  = BASE / "step7_representation_diagnostics_scgpt_sex"
STEP8_DIR  = BASE / "step8_sex_conditioned_disease_scgpt"
...
SEX_COLORS = {"female": "#E91E63", "male": "#1976D2",
              "Female": "#E91E63", "Male": "#1976D2"}

DATASET_ORDER = [
    "Proportional_1999", "BalancedAugmented_1413Each",
    "BalancedUpsampled_1413Each", "Downsampled_586Each",
]
...
UMAP_DATASETS = {
    "Proportional_1999":          "ILD_Sex_Pilot_Proportional_1999_scgpt",
    "BalancedAugmented_1413Each": "ILD_Sex_Pilot_BalancedAugmented_1413Each_scgpt",
    "BalancedUpsampled_1413Each": "ILD_Sex_Pilot_BalancedUpsampled_1413Each_scgpt",
    "Downsampled_586Each":        "ILD_Sex_Pilot_Downsampled_586Each_scgpt",
}
...
def generate_individual_umaps():
    ...
    for ds_key, ds_stem in UMAP_DATASETS.items():
        ...
        if "sex" in adata.obs.columns:
            ...
            _umap_scatter(ax, coords, adata.obs["sex"].astype(str).values,
                          SEX_PALETTE_UMAP, pt_size,
                          f"{display}\nColored by Sex (scGPT Embedding)")
            plt.tight_layout()
            save(fig, f"umap_scgpt_{_safe(display)}_sex.png")
...
def fig1_scib():
    ...
    for metric, (_, short, title, subtitle) in SCIB_METRIC_MAP.items():
        ...
        _single_bar_figure(
            title=f"Fig 1 — scIB Embedding Quality (scGPT SEX)\n{title}",
...
def fig2_step4a():
    path = STEP4A_DIR / "step4a_downstream_results_sex_AR_EOS_scgpt.csv"
    if not path.exists():
    ...
    for metric, panel_title, ylabel, fname in panels:
        ...
        fig.suptitle(f"Fig 2 — Disease Prediction Strategies (scGPT SEX)\n{panel_title}",
...
def fig3_step4b():
    path = STEP4B_DIR / "step4b_results_sex_labeled_scgpt.csv"
    if not path.exists():
    ...
    for ds in datasets:
        ...
        fig, ax = plt.subplots(figsize=(14, 7))
        fig.suptitle(f"Fig 3 — Model Robustness (scGPT SEX)\nDataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
        if data_vals:
...
def fig4_learning_curves():
    path = STEP5_DIR / "step5_learning_curves_scgpt_sex.csv"
    if not path.exists():
    ...
    for ds in datasets:
        ...
        fig.suptitle(f"Fig 4 — Learning Curves (scGPT SEX)\nDataset: {ds_label(ds).replace(chr(10), ' ')}",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig5_step6():
    path = STEP6_DIR / "step6_per_sex_diagnostics_scgpt.csv"
    if not path.exists():
    ...
    metric_titles = {
        "silhouette": "Disease Silhouette Score Delta", "knn_mixing": "kNN Sex Mixing Delta",
    ...
    for grp in df.index.tolist():
        ...
        for metric in metrics:
            ...
            fig.suptitle(f"Fig 5 — Per-Sex Fairness Deltas (scGPT SEX)\nGroup: {grp.title()}   Metric: {metric_titles.get(metric, metric)}",
...
def fig6_step7():
    path = STEP7_DIR / "step7_per_sex_diagnostics_scgpt.csv"
    if not path.exists():
...
def fig7_step8():
    per_sex_path = STEP8_DIR / "step8_per_sex_disease_prediction_scgpt.csv"
    worst_path   = STEP8_DIR / "step8_worst_sex_summary_scgpt.csv"
    ...
    fig.suptitle("Fig 7A — Sex-Conditioned Disease Prediction (scGPT SEX)\nPer-Sex Accuracy by Dataset Variant",
                 fontsize=17, fontweight="bold", y=1.02)
    ...
    if not worst.empty:
        ...
        fig.suptitle("Fig 7B — Worst-Performing Sex Group Accuracy (scGPT SEX)",
                     fontsize=17, fontweight="bold", y=1.02)
    ...
    if "delta_acc_vs_prop" in fem_rows.columns and not fem_rows.empty:
        ...
        fig.suptitle(f"Fig 7C — {UNDERREP_GROUP.title()} Accuracy Delta vs Proportional (scGPT SEX)",
                     fontsize=17, fontweight="bold", y=1.02)
...
def fig8_summary_heatmap():
    ...
    p = STEP4A_DIR / "step4a_downstream_results_sex_AR_EOS_scgpt.csv"
    if p.exists():
        df4  = pd.read_csv(p)
    ...
    fig.suptitle("Fig 8 — Summary Heatmap (scGPT SEX)\nGreen = best   Red = worst   Raw values shown",
...
def main():
    print("\nSTEP 9 — Visualization Pipeline (SEX, scGPT)")
