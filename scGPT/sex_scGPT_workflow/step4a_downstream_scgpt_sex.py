#!/usr/bin/env python3
"""
STEP 4a — Downstream Disease Prediction (SEX, scGPT)
...
OUTFILE_TXT = OUTDIR / "step4a_downstream_results_sex_AR_EOS_scgpt.txt"
OUTFILE_CSV = OUTDIR / "step4a_downstream_results_sex_AR_EOS_scgpt.csv"
...
def extract_features(adata, label_col, grp_col):
    ...
    g = canonicalize(adata.obs[grp_col])
...
def main():
    ...
    with open(OUTFILE_TXT, "w") as fh:
        fh.write("STEP 4a -- Downstream Disease Prediction (SEX, scGPT)\n" + "="*80 + "\n")
        for dname, fpath in DATASETS.items():
            if not fpath.exists():
                log(f"  Skipping {dname}"); continue
            log(f"Loading {dname}")
            ...
            for model_name, model_tmpl in MODEL_TEMPLATES.items():
                for strat_name, flags in STRATEGIES.items():
                    ...
                    ct_acc, ct_f1, ct_worst, ct_worst_acc, ct_per = evaluate(
                    ...
                    for grp, a in per_group.items():
                        row[f"per_sex_disease_acc_{grp}"] = a
                    rows.append(row)
