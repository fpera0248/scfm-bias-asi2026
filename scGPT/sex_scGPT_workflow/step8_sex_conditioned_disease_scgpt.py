#!/usr/bin/env python3
"""
STEP 8 — Sex-Conditioned Disease Prediction (SEX, scGPT)
...
Changes vs ethnicity scGPT version:
  ...
  [SEX 5] Reference      -> Proportional_1999
"""
...
OUTDIR      = BASE / "step8_sex_conditioned_disease_scgpt"
...
def main():
    log("="*70)
    log("STEP 8 -- Sex-Conditioned Disease Prediction (SEX, scGPT)")
    log("="*70)
    ...
    per_sex_df.to_csv(OUTDIR / "step8_per_sex_disease_prediction_scgpt.csv",   index=False)
    worst_sex_df.to_csv(OUTDIR / "step8_worst_sex_summary_scgpt.csv",          index=False)
    underrep_df.to_csv(OUTDIR / "step8_underrepresented_sex_summary_scgpt.csv", index=False)

    lines = [
        "STEP 8 -- SEX-CONDITIONED DISEASE PREDICTION (SEX, scGPT)", "="*70, "",
