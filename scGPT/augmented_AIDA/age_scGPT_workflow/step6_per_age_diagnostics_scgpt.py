#!/usr/bin/env python3
"""
STEP 6 — Per-Age Fairness Diagnostics (AGE, scGPT)
...
BASE = pathlib.Path(
    "/oscar/home/fperalta/data/fperalta/scGPT/augmented_AIDA/age_scGPT_workflow"
)

LABELED_DIR = BASE / "step2a_embeddings"
OUTDIR      = BASE / "step6_outputs_age_scgpt"             # [AGE 5]
OUTDIR.mkdir(exist_ok=True)

OUTPUT_BASE = "AIDA_Age_Pilot"                                   # [AGE 1]

FILES = {                                                       # [AGE 3]
    "Proportional_2498":          LABELED_DIR / f"{OUTPUT_BASE}_Proportional_2498_AGE_scgpt.h5ad",
    "BalancedAugmented_747Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedAugmented_747Each_AGE_scgpt.h5ad",
    "BalancedUpsampled_747Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedUpsampled_747Each_AGE_scgpt.h5ad",
    "Downsampled_230Each":         LABELED_DIR / f"{OUTPUT_BASE}_Downsampled_230Each_AGE_scgpt.h5ad",
}
...
def main():
    ...
    out_csv = OUTDIR / "step6_per_age_diagnostics_scgpt.csv"
    pivot.to_csv(out_csv)
    log(f"\nCSV saved -> {out_csv}")

    out_txt = OUTDIR / "step6_per_age_diagnostics_report_scgpt.txt"
    lines = [
