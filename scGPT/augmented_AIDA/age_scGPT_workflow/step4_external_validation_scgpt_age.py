#!/usr/bin/env python3
"""
STEP 4 -- External Validation Disease + Cell-Type Classification (AGE, scGPT)

Changes vs ethnicity scGPT version:
  [SEX 1] BASE/OUTDIR    -> age_scGPT_workflow
  [SEX 2] EMB_KEY        -> X_scGPT
  [SEX 3] PILOTS         -> sex filenames
  [SEX 4] VALIDATION     -> AIDA_Age_External_Validation_10000_scgpt.h5ad
  [SEX 5] GROUP_COL      -> "age_bin_10yr"
  [SEX 6] DISEASE_GROUPS -> {"male", "female"}
"""
...
OUTDIR      = BASE / "step4_external_validation_scgpt_age"
...
VALIDATION_FILE = BASE / "step2a_embeddings" / "AIDA_Age_External_Validation_10000_scgpt.h5ad"

CSV_FILE = OUTDIR / "step4_external_validation_results_age_scgpt.csv"
TXT_FILE = OUTDIR / "step4_external_validation_log_age_scgpt.txt"
...
def load_adata(name, path):
    ...
    if EMB_KEY not in ad.obsm:
        raise RuntimeError(f"{name}: missing {EMB_KEY}")
...
def main():
    ...
    log("="*70)
    log("STEP 4 -- External Validation Classification (AGE, scGPT)")
    log("="*70)
    ...
    for pname, ppath in PILOTS.items():
        log(f"\n{'='*70}\nPilot: {pname}")
        try:
            X_pilot, age_pilot, dis_pilot, ct_pilot = load_adata(pname, ppath)
    ...
    log(f"\n{'='*70}\nSTEP 4 COMPLETE (AGE, scGPT)")
    summary_cols = ["pilot", "model", "strategy", "disease_accuracy", "disease_macro_f1",
