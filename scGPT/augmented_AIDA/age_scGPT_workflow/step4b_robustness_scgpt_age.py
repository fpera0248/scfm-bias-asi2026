#!/usr/bin/env python3
"""STEP 4b — Robustness Stress Test (AGE, scGPT)"""
...
OUTDIR = BASE / "step4b_model_robustness_tests_age_scgpt"; OUTDIR.mkdir(exist_ok=True)
CSV_FILE = OUTDIR / "step4b_results_age_labeled_scgpt.csv"
TXT_FILE = OUTDIR / "step4b_full_log_age_labeled_scgpt.txt"

logger = logging.getLogger("STEP4B_AGE_SCGPT"); logger.setLevel(logging.INFO)
fmt = logging.Formatter("%(asctime)s | %(levelname)s | %(message)s")
for h in [logging.FileHandler(TXT_FILE), logging.StreamHandler()]:
    h.setFormatter(fmt); logger.addHandler(h)

warnings.filterwarnings("always")
logger.info("========== STEP 4b START (AGE, scGPT) ==========")

EMB_KEY = "X_scGPT"; LABEL_KEY = "disease"
...
logger.info(f"========== STEP 4b COMPLETE (AGE, scGPT) ==========")
logger.info(f"CSV -> {CSV_FILE}"); logger.info(f"Runtime -> {total_min:.2f} min")
