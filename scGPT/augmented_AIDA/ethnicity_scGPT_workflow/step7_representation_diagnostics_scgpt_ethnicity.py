#!/usr/bin/env python3
"""
STEP 7 — Representation Quality & Fairness Diagnostics (ETHNICITY, scGPT)
...
Changes vs Geneformer ethnicity version:
  ...
  [SCGPT 4] Reference    -> Proportional_2500
"""
...
FILES = {
    "Proportional_2500":          LABELED_DIR / f"{OUTPUT_BASE}_Proportional_2500_scgpt.h5ad",
    "BalancedAugmented_779Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedAugmented_779Each_scgpt.h5ad",
    "BalancedUpsampled_779Each": LABELED_DIR / f"{OUTPUT_BASE}_BalancedUpsampled_779Each_scgpt.h5ad",
    "Downsampled_92Each":         LABELED_DIR / f"{OUTPUT_BASE}_Downsampled_92Each_scgpt.h5ad",
}
...
def celltype_linear_probe(ad):
    ...
    return pd.Series({eth: round(float(f1_score(sub["y_true"], sub["y_pred"], average="macro")), 4)
                      ...
                      for sub in [df[df[ETH_KEY] == eth]]
                      if sub["y_true"].nunique() >= 2})

def main():
    log("="*70)
    log("STEP 7 -- Representation Quality & Fairness Diagnostics (ETHNICITY, scGPT)")
    log("="*70)
    ...
    if "Proportional_2500" not in datasets:
        raise RuntimeError("Proportional_2500 required as reference baseline.")
    ...
    df_all.to_csv(OUTDIR / "step7_per_ethnicity_diagnostics_scgpt.csv", index=False)
    ...
    lines = [
        ...
        pivot.to_string(), "", "="*70, "DELTA SUMMARY (vs Proportional_2500)", "="*70,
        pivot[delta_cols].sort_index().to_string() if delta_cols else "No delta columns produced.",
    ]
    (OUTDIR / "step7_summary_scgpt_ethnicity.txt").write_text("\n".join(lines))
    log("STEP 7 COMPLETE")
